...
- `銘柄コックピット` には `外部資料取得（明示許可）` の first UI slice がある。`外部通信を許可する` を選んだ場合だけ Yahoo Finance provider profile / news を取得する。取得本文は session-local RAG store に一時登録してその場の summary / score / news 表示に使い、画面には provider、fetched_at、published_at、source URL、warnings、短い要約を表示する。既定では Markdown / manifest JSON を保存しない。
...
Current implementation note: `ExternalResearchFetchService` and `YahooFinanceResearchAdapter` are wired to the Cockpit Research Evidence panel as a first opt-in UI slice. The default UI flow registers external payload text in the session-local RAG store, rebuilds chunks for the current session, and does not write fetched payload Markdown or manifest files. A future explicit archive action may opt into persistence separately.

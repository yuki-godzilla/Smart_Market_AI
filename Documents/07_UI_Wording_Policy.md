...
- Direction Signal は「上がる銘柄/下がる銘柄」ではなく、「上昇気配」「下降警戒」「方向感」「深掘り候補」として表現する。
...
| `Direction Signal` / `方向感` | 上昇気配と下降警戒の差分を整理した深掘り用シグナル | 売買推奨ではなく、比較・確認の優先度として説明する |
| `上昇気配` | 予測上昇率、上向きモデル割合、上向きモメンタム、ばらつきの小ささを統合した補助指標 | 「上がる」と断定せず、上向きシグナルとして説明する |
| `下降警戒` | 予測下落率、下向きモデル割合、下向きモメンタム、ばらつきの小ささを統合した補助指標 | 売り推奨ではなく、リスク確認候補として説明する |

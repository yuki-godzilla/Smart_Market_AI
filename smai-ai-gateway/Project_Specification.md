...
| SMAI LLM Factor structured extraction support | 将来範囲 | RAG / News / IR 由来の定性材料を構造化特徴量へ変換する補助 | Gateway は provider / prompt 境界に留め、`LLMFactorResult` schema、cache、backtest、UI 統合は SMAI 本体側で扱う | 親側 roadmap / [docs/prompt_policy.md](docs/prompt_policy.md) |
...
- `SMAI LLM Factor` 向けの structured extraction endpoint / prompt profile は未実装。domain schema、cache、backtest、UI 統合は SMAI 本体側の将来範囲。
...
| SMAI LLM Factor | 将来範囲 | LLM を最終予測器ではなく、source-bound qualitative feature generator として使う構想。Gateway 側は provider / prompt 実行境界に限定する。 |

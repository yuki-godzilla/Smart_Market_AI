...
実point-in-time材料の継続収集・LLM risk signal生成も同水準のrun contractへまとめた。sealed origin以前に
公開・利用可能・初回保存された銘柄一致材料だけを使い、hash付き追記専用signal、引用、Gateway / validation失敗、
検証済みbackupを保存する。初回は材料保存開始がoriginより後だったためeligible 0件・Gateway呼び出し0回だった。

次のbackend作業は、LLM GatewayとMarketData Providerのtimeout / partial failure / recovery観測、API / 保存 / job境界を
backend完了gateで横断確認することである。UIの進捗表示はbackend contractが安定した後に行う。

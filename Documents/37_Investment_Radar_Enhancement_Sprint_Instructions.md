...
> UX・可視化の後続P0要件は、[39_Investment_Radar_UX_Visualization_Requirements.md](39_Investment_Radar_UX_Visualization_Requirements.md) を正本とする。本書の候補・RAG・Gateway境界は維持し、画面構成と用語が異なる場合は同要件書を優先する。
...
- 各候補カードは、材料種別、ニュース鮮度、独立根拠数、Watchlist一致、確認の順番を短く示す。材料種別だけから好材料・注意材料などの方向を推定してはならず、方向を表示するのは別の根拠で検証できる場合に限る。
...
- 候補クリック時に、次を一つの詳細surfaceで表示する。
  ...
  - 「材料種別」: 決算、業種・テーマ、リスクなど。記事内容の方向が未検証なら `方向未確認` とする
  - 「データの状態」: ニュース鮮度、銘柄DB/価格の取得状態、RAG確認状態
  - 「次の確認」: `根拠資料を確認`、Cockpitを開く、外部資料を確認する場合の明示操作
...
- 初期候補は各由来レーン3〜4件、全体で最大12件を目安にし、残りは `あと N 件を見る` で明示展開する。PCでは選択候補の詳細へ常に到達できるようにし、候補一覧だけが長いときに大きな空白を残さない。iPadでは一覧と詳細を狭い2列に固定せず、iPhoneでは選択候補の直後またはダイアログに詳細を出す。表・一覧だけに必要な内部スクロールは許可するが、ページ全体の横スクロールを発生させない。

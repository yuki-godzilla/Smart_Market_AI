...
Research RAG は通常検証を local-first / deterministic-first に保つが、実運用の価値は資料・ニュースの鮮度に大きく依存する。外部取得が明示 assignment された場合は、opt-in adapter、source URL、published_at、fetched_at、freshness warning を備えた current external evidence を優先する。取得本文は既定では保存せず、その回の RAG summary / score / 表示で一時参照する。
...
- `銘柄コックピット` には `外部資料取得（明示許可）` の first UI slice がある。`外部通信を許可する` を選んだ場合だけ Yahoo Finance provider profile / news を取得する。現在の初期実装は `data/research_docs/external_cache/` へ Markdown + manifest JSON を保存するが、方針としてはこれを既定動作にしない。次の修正で、取得本文は session-local RAG store に一時登録してその場の summary / score / news 表示に使い、画面には provider、fetched_at、published_at、source URL、warnings を表示する形に戻す。
...
- Phase 21.6 は `外部資料を取得` / `外部資料を参照` を explicit opt-in action として追加し、EDINET / TDnet / IR site / provider profile などの外部資料取得 adapter を実現する候補とする。
...
Current implementation note: `ExternalResearchFetchService` and `YahooFinanceResearchAdapter` are wired to the Cockpit Research Evidence panel as a first opt-in UI slice. The first implementation still saves external payloads and manifest files under `data/research_docs/external_cache/`; this is now treated as implementation debt. The next code slice should switch external fetch to transient-by-default session-local evidence and remove default payload/manifest persistence from the UI flow.

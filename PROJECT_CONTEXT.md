...
- Next focus is to continue Phase 21 Advanced Research RAG after deterministic query expansion, structured extraction, template grounded answer, retrieval quality, evidence reranker, first UI / Decision Report display, optional vector / hybrid contract/scoring, keyword-fallback hybrid retrieval wrapper, and in-memory local vector store slices: file-backed vector cache or local embedding generation remains next. Phase 21.5 Stock News RAG now covers selected-symbol cockpit news evidence from local URL-backed news documents only. Phase 21.6 / 21.7 are roadmap child phases for explicit opt-in external research document fetch and external stock news fetch with cache/manifest and network-free tests. Research Score, broader Investment News dashboard, Assistant, distribution readiness, and Execution/Broker integration remain planned or future scope unless explicitly assigned.

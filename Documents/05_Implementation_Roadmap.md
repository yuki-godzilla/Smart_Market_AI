...
- `SMAI LLM Factor` の backtest / Ranking 参考カラム / optional forecast integration。schema、1銘柄 Cockpit 参考表示、cache / reproducibility の初期 slice は実装済み。初期段階では既存予測モデル、Ranking、Investment Score へ混ぜない
...
- Phase 24 Assistant は deterministic backend、Cockpit / Ranking 向け floating `SMAI Copilot` UI、Gateway request / response schema、`smai-ai-gateway/` scaffold まで実装済み
...
| 2 | `SMAI LLM Factor` Phase C-E | 再現性と有効性を検証する | cache / reproducibility 初期 slice は実装済み。次は backtest、必要に応じて Ranking 参考カラム | 有効性確認まで予測モデルへ混ぜない |
...
状態: Phase A-C の最初の SMAI 側 MVP / cache slice 実装済み。`backend/llm_factor` の Pydantic schema、deterministic fake service、file-backed cache、銘柄コックピットの `AI材料分析` 参考表示、cache status / generated_at / expires_at / model / prompt version / source hash の表示まで。既存予測モデル、Ranking、Investment Score には初期段階では混ぜない。
...
- `SMAI LLM Factor` Phase E として、backtest の評価窓 / 指標 / fixture をどう定義し、その後 Ranking 参考カラムをどの範囲で出すか

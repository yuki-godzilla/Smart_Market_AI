...
class RankingInterpretationConfig(StrictConfigModel):
    ...
    prompt_version: str = Field(default="ranking_interpretation_mvp.v1", min_length=1)
    schema_version: str = Field(default="ranking_interpretation.v1", min_length=1)
    cache_enabled: bool = True
    cache_ttl_seconds: int = Field(default=21600, gt=0, le=86400)
    max_candidates: int = Field(default=5, gt=0, le=5)
    max_sector_groups: int = Field(default=4, gt=0, le=6)

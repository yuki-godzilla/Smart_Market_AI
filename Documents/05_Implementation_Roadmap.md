...
- R10.4: CompanyResearchSummary / 企業リサーチレポート。初期 slice は実装済み。`CompanyResearchReport`、`ResearchBrief`、`ExternalResearchFetchResult`、`StockNewsReport`、`InvestmentInsight` から、企業概要、事業内容、地域、規模感、主要定量指標、IR資料取得状況、最新ニュース、AI読み取りメモ、不足している重要情報を生成する。根拠資料画面ではこれを主役にし、投資判断メモは後段の補助情報に下げる。
...
- Cockpit Research Summary の主表示順は、企業リサーチサマリー -> 定量情報サマリー -> IR情報サマリー -> 最新ニュースサマリー -> AI読み取りメモ / 企業理解の確認ポイント -> 3カラム確認情報（確認できた情報 / 注意して読む情報 / 不足している情報）-> folded 根拠確認（AI整理メモ / 読み方サマリー / 会社概要 / 確認できた事実 / 公式資料で未確認）とする。出典カード、Research Score、件数、confidence、詳細データは必要時に開く補助情報へ下げる。

...
| Phase 29 | 🟨 **MVP済み / 継続拡張** | Phase 29-A Cockpit情報設計整理は実装済み。確認レポート草案支援は後続。 |
| Phase 30 | ⬜ **後続 / 未着手** | LLM Factor検証と段階統合。 |
...
- 🟨 **MVP済み / 継続拡張**: Phase 29-A Cockpit Information Architecture Cleanup は実装済み。Cockpitの主導線を `03 AI解釈メモ` -> `04 スコア・リスクの内訳` -> `05 根拠資料` -> `06 確認レポート` -> `07 詳細データ` に整理し、AI材料分析は根拠資料配下の参考表示、provider / model / cache などの実行情報は折りたたみ詳細に寄せた。Forecast / Ranking / Investment Score / LLM Factor / Research取得ロジックは変更しない。Phase 29-B以降は確認レポート草案支援を扱う。

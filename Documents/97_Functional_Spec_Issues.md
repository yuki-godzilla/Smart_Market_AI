...
| FS-006a | Research Summary / ResearchBrief | Research Summary が生データ羅列で読めない | `ResearchBriefBuilder` が provider profile、news、TDnet trace、evidence を `ResearchBrief` / `ResearchMetric` に変換し、Cockpit / Ranking Research Summary 冒頭へ AI整理メモ、確認ポイント、定量指標、確認不足、次の確認資料を表示し、出典カードやResearch Scoreは展開式で確認する | 外部LLMは使わず local rule-based 生成を維持し、source confidence は情報源の信頼度として説明する。Research Score は調査メモの後ろへ寄せ、ranking order は変えない | ユーザーが何を確認すべきか分からない、provider raw fields を投資根拠と誤読する | UX readability | High | In review | 主表示を要点に絞り、出典カード・Research Score・外部参照ソース・詳細表は折りたたみへ移動済み。UI/browser確認で表示順と折り返しを確認 |

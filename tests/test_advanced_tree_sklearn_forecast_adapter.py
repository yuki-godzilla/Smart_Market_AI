...
def test_advanced_tree_sklearn_adapter_predicts_forward_return():
    ...
    assert 61 in SUPPORTED_ADVANCED_TREE_SKLEARN_HORIZONS
...
def test_advanced_tree_sklearn_adapter_supports_horizon_above_former_limit():
    result = AdvancedTreeSklearnForecastAdapter().forecast(_bars(180), horizon_days=75)

    assert result.horizon_days == 75


def test_advanced_tree_sklearn_adapter_rejects_non_positive_horizon():
    with pytest.raises(ValueError, match="horizon_days"):
        AdvancedTreeSklearnForecastAdapter().forecast(_bars(80), horizon_days=0)

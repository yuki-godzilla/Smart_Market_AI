...
- `multi_factor`: `Upside x Downside Watch Map`。棒グラフで総合スコア、散布図で上昇気配と下降警戒を確認する。
- `upside_signal`: `Upside x Downside Watch Map`。`上昇気配` と `下降警戒` を主軸にして確認する。
- `dividend` / `sustainable_income`: `Fit x Risk Map`、列不足時は `Score x Risk Map`
- `growth` / `quality_growth` / `small_growth`: `Fit x Upside Signal Map`、列不足時は `Score x Upside Signal Map`
...
- `trend` / `momentum`: `Momentum x Upside Signal Map`、列不足時は `Score x Upside Signal Map`
...
Symbol Cockpit は、Summary / KPI の直後に `02 Price & Forecast` を置き、価格・予測チャートを1銘柄深掘りの主役として扱う。ランキングで使う `上昇気配・下降警戒` は Analysis KPI に集約し、チャート直後の `03 Signal Reading / シグナル読み取り` ではカードを重複させず、SMAI Insight と確認ポイント表で状況を読む。`04 Score Breakdown` は値動きと2指標を見たあとに「なぜこの評価になっているか」を確認する位置づけにする。

- `01 Summary / Symbol Cockpit` で対象、Provider、期間、主要スコアを確認する。
- `Analysis KPI` でスコア、上昇気配、下降警戒、Data Confidence、Risk を把握する。
- `02 Price & Forecast` で実績価格と予測レンジを確認する。
- `03 Signal Reading / シグナル読み取り` で、上昇気配・下降警戒の優勢/拮抗、予測変化率、モデル方向一致、予測のばらつきを確認する。
- `04 Score Breakdown`、期間別評価、Review Memo、Confirmation Summary の順で根拠を読む。
...
- 短期は直近反応と短期 forecast に寄せる。
- 標準は20日/60日系の予測材料、上昇気配・下降警戒、Risk の初期確認に使う。
- 中期はトレンドとノイズのバランス確認に寄せる。

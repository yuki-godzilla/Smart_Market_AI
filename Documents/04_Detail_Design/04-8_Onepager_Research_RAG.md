...
- 外部ニュース取得は explicit opt-in adapter とし、通常 checks / CI は network、live scraping、external LLM に依存させない。

### Phase 21.6 / 21.7 External Fetch Boundary

- Phase 21.6 は `AI調査を更新` の標準処理として、EDINET / TDnet / IR site / provider profile などの外部資料取得 adapter を実現する候補とする。
...
### ResearchBrief Local Summary Direction / ローカル調査メモ化

Next Research Summary work should add a display-only `ResearchBrief` layer between `CompanyResearchReport` / evidence and the Streamlit UI. The goal is to stop showing provider dumps or raw evidence as the main experience, and instead present a readable investment research memo generated by deterministic local rules. External LLM / OpenAI API integration is deferred; normal checks remain network-free and LLM-free.
...
`ResearchBriefBuilder` should:

- compress Yahoo Finance / provider profile into company overview and business description, not show raw fields such as Provider Symbol, Quote Type, Exchange, Currency, Sector, or Industry in the normal view;
- separate quantitative metrics from qualitative topics;
- extract first-slice metrics with local regex / keyword rules: revenue, operating income, net income, EPS, dividend, PER, PBR, ROE, and market capitalization;
- put unavailable important metrics into `missing_metrics`;
- classify qualitative evidence into growth, performance, shareholder return, risk, market theme, positive candidate, caution candidate, and missing evidence buckets using local keywords;
- set source confidence by source role: official IR / TDnet / EDINET and company IR are `high`, provider profile and news are `medium`, keyword-only extraction is `low`;
- produce `missing_points` for missing official IR, missing latest earnings, missing outlook, missing dividend policy, missing risk description, stale news, insufficient primary sources, and missing key metrics;
- produce `next_actions` such as checking TDnet, company IR, EDINET, latest earnings material, dividend policy, and risk factors;
- keep raw evidence / provider field dumps only in detail rows.

Preferred Cockpit Research Summary order:
...
Research Score should not be the first block. It should appear after the local AI整理メモ or inside detail/context as a reference score for evidence coverage, freshness, and reliability.

Local AI整理メモ template examples:

- Provider profile only: `現時点では、会社概要などの補助情報は確認できていますが、公式IR・決算資料などの一次情報は不足しています。投資判断前には、公式資料による裏取りが必要です。`
- Official disclosure exists: `公式開示情報が確認できています。決算内容、業績修正、配当方針などの一次情報を優先して確認してください。`
- Evidence limited: `現時点では、確認できた根拠が限られています。最新決算、配当方針、リスク要因を公式資料で確認してください。`

Additional tests for this slice should cover provider-profile compression, hidden provider raw fields in normal display, official-source coverage, provider-only caution, metric extraction / missing metrics, confidence labels by source type, keyword-based topic classification, source-card check purpose, and UI display order.
...
* 外部取得（EDINET / TDnet / IR RSS / News API 等）は adapter として段階的に追加し、`AI調査を更新` の標準 source にする。

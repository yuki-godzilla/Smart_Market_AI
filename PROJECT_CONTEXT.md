...
Smart Market AI is a Python-based investment support project that combines market-data ingestion, feature generation, screening, deterministic baseline forecasting, model-informed Investment Score, portfolio/risk checks, Streamlit UI, Decision Report context, and a Phase 20 local Research RAG evidence slice.
...
For Research RAG / News RAG, local deterministic checks remain mandatory, but freshness is a product priority: when external RAG fetch work is explicitly assigned, current opt-in external sources should be prioritized as transient live evidence with source URL / provider / published_at / fetched_at / freshness warnings. Fetched source text should not be retained by default; persistent archive should require a separate explicit user action.
...
- Research RAG Phase 20 local evidence slice is implementation-complete. Phase 21 is planned as Advanced Research RAG for query expansion, structured evidence extraction, optional vector / hybrid search, evidence reranking, and grounded answer generation. Phase 21.5 Stock News RAG has a first local deterministic slice for selected-symbol cockpit news evidence from registered `source_type=news` documents with source URLs; it does not change ranking order. Phase 21.6 / 21.7 have a first opt-in Cockpit UI slice for Yahoo Finance external profile / news fetch: the user must allow external communication. The first implementation currently saves fetched Markdown + manifest JSON under `data/research_docs/external_cache/`, but this is now treated as implementation debt; target behavior is transient-by-default live evidence that updates the session-local RAG store for the current summary / score / display pass without retaining fetched documents. Phase 22 Research Score has a deterministic service slice, optional Investment Score numeric input with disabled-by-default `research` weight, Cockpit / Ranking Research Summary display rows, selected-candidate breakdown context, and a first Cockpit Decision Report section slice; ranking order integration remains planned.
...
- Long-term Research RAG storage direction: `data/research_docs/` remains user-controlled storage for manual uploads, private notes, and deterministic fixtures. External live sources should not auto-populate it by default; archive/save should be a separate explicit user action with cleanup behavior.
...
- Next focus is to correct Phase 21.6 / 21.7 external fetch to transient-by-default behavior, or continue Phase 22 Research Score carefully while preserving unchanged default ranking order. Phase 21.5 Stock News RAG covers selected-symbol cockpit news evidence from local URL-backed news documents. Phase 21.6 / 21.7 have a first Cockpit UI slice for explicit opt-in Yahoo Finance external research/news fetch, but the next code slice should remove default payload/manifest persistence and use session-local live evidence for summary / score / display only. EDINET / TDnet / company IR site adapters, explicit save/archive UI, broader Investment News dashboard, Assistant, distribution readiness, and Execution/Broker integration remain planned or future scope unless explicitly assigned.

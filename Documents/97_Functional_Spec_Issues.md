...
| FS-006a | Research Summary / ResearchBrief | Research Summary が生データ羅列または取得状態の説明に寄り、ユーザーが知りたい事業・IR・指標が見えにくい | `ResearchBriefBuilder` が `ResearchFactSummary` を優先入力として、provider profile、news、TDnet trace、evidence を AI整理メモ、3つの確認ポイント、定量指標、確認不足、次の確認資料へ変換し、Cockpit / Ranking Research Summary に表示する | 主表示は `会社概要` / `確認できた事実` / `公式資料で未確認` に集約し、事業概要、主要事業、確認済みIR / 公式資料、主要定量指標、直近イベント、未確認項目を source-backed fact として表示する。良材料候補 / 注意材料候補、出典カード、Research Score、件数、confidence は補助情報として扱う。外部LLMは任意の後続候補で、まず local rule-based 生成を維持する | ユーザーが何を確認すべきか分からない、provider raw fields を投資根拠と誤読する、取得件数だけが成果に見える | UX readability | High | In review | 初期 `ResearchFactSummary` slice と3ブロック主表示は実装済み。次は実画面で情報量、折り返し、折りたたみ境界を確認 |

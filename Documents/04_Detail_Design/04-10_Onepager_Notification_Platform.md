...
- N6第6A sliceでは、`favorite_move_alert`の市場時間判定を端末ローカル時刻の一律条件から、保存済み`market` / `asset_type`だけで解決する銘柄単位のregular-session policyへ移す。日本株は`Asia/Tokyo`の09:00–11:30 / 12:30–15:30、米国株は`America/New_York`の09:30–16:00を用い、DSTはIANA timezoneで扱う。市場不明、非対応asset、週末、時間外はsampleへfallbackせずsafe reasonでskipする。schedulerは同一のtimezone付き`evaluated_at`をadapterへ渡し、dry-runと実行で境界判定を一致させる。tickerから市場を推測せず、Provider取得、cache/profile書込み、Ranking / Forecast / Score変更は行わない。祝日・短縮取引の明示カレンダーはN6第6Bとして未導入であり、regular weekday sessionを休場確認済みと扱わない。

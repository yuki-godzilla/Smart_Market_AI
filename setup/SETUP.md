...
dev 依存として Playwright と Chromium も導入され、Streamlit の実画面 smoke に使えます。
...
Streamlit の実画面 smoke を行う場合は、dev 依存の Playwright + Chromium を使えます。
通常の local checks / CI は引き続き外部ネットワークやブラウザ起動に依存しません。
...
### Playwright の Chromium が見つからない

既存の仮想環境だけ更新した場合は、依存インストールとは別に browser binary の取得が必要です。

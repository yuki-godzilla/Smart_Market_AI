...
- `free_chat` は実用速度を優先する軽量経路として扱う。SMAI 親側では Tool Layer / RAG / news / symbol-specific context / chat-history payload を送らず、Gateway 側では短い prompt、短い timeout / max tokens、内部推論を表示しない cleaning / fast-path fallback を使う。

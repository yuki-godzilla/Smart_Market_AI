...
- 将来の `SMAI LLM Factor` では、Gateway は provider 呼び出しと prompt 実行の境界に留める。LLM factor の domain schema、source hash、file-backed cache、deterministic backtest evaluator、Cockpit / Ranking 参考表示は SMAI 本体側の初期 slice で実装済みで、broader historical backtest、cache policy expansion、UI 統合拡張も SMAI 本体側で扱う

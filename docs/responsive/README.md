...
Cockpit、Ranking、My Radar、投資レーダー、SMAI Assistant を、PC の情報密度を保ちながら iPhone / iPad でも安全に閲覧・操作できるようにします。投資判断、スコア、予測、データ取得、AI、RAG のロジックは変更対象外です。
...
- Streamlit columns: iPad では折り返し、iPhone では縦積み
...
3. Cockpit → Ranking → My Radar → 投資レーダー → SMAI Assistant の順に確認します。
...
画面名は `cockpit`、`ranking`、`my_radar`、`investment_radar`、`assistant` です。自動取得環境がない場合は Chrome DevTools の Capture screenshot を使います。
...
.\venv_SMAI\Scripts\python.exe -m pytest tests/ui/test_responsive_cockpit_smoke.py tests/ui/test_responsive_ranking_smoke.py tests/ui/test_responsive_my_radar_smoke.py -q
Remove-Item Env:SMAI_RUN_RESPONSIVE_SMOKE
```

URLを変更する場合は `SMAI_STREAMLIT_URL` も指定します。テストは4 viewportの横はみ出し、Streamlit例外、画面見出し、ボタンの存在を検査し、Cockpit / Ranking / My Radarスクリーンショットを保存します。

...
- SMAI 本体側の `SMAI LLM Factor` 初期 schema / fake service / file-backed cache / deterministic backtest evaluator / Cockpit 参考表示は親プロジェクト側で実装済み。Gateway 側にはまだ構造化抽出 endpoint を追加しない。
...
- `LLMFactorResult`、factor schema、source hash、file-backed cache、deterministic backtest evaluator は SMAI 本体側の初期 slice で実装済み。Ranking 参考表示、broader historical backtest、cache policy expansion、UI 統合拡張も SMAI 本体側で扱い、Gateway から SMAI module は import しない

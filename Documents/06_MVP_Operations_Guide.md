...
- Research Summary は、外部LLMを使わず、local rule-based `CompanyResearchSummary`、`ResearchBrief`、`InvestmentInsight`、`InvestmentQuestionSummary` へ変換してから表示する。通常表示では provider profile の生フィールド羅列を出さず、最初に `企業リサーチサマリー` として企業概要、事業内容、地域展開、規模感、直近の注目ポイントを表示する。続けて `定量情報サマリー` に売上高、営業利益、純利益、EPS、PER、PBR、ROE、配当利回り、時価総額、`IR情報サマリー` に決算短信、決算説明資料、有価証券報告書、適時開示、中期経営計画、配当・自社株買い、`最新ニュースサマリー` に直近ニュースと業績影響ヒントを表示する。`AI読み取りメモ`、`企業理解の確認ポイント`、確認できた情報 / 注意して読む情報 / 不足している情報は後段に置く。AI整理メモ、読み方サマリー、詳細な確認ポイント、出典カード、Research Score、外部参照ソース、詳細表は必要なときに展開して確認する。上部の operation card は `企業リサーチレポートを更新しました` / `追加確認` に寄せ、抽出指標数、出典カード数、Research Score は折りたたみ側に下げる。

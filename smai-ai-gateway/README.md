...
将来 `SMAI LLM Factor` の構造化特徴量生成を Gateway 経由で行う場合も、SMAI domain schema と初期 Cockpit 参考表示は SMAI 本体側に残し、cache、backtest、UI 統合拡張も SMAI 本体側で扱います。Gateway は provider 呼び出しと prompt 実行の境界に留めます。

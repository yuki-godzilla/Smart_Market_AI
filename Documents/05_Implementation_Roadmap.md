...
- Research RAG の `ResearchFactSummary` fact layer、追加 external source adapter、Research Score の ranking 統合
...
- R10: ResearchFactSummary / User-facing Fact Layer。次の planned slice として、`ResearchBrief` の前段に source-backed fact contract を追加し、取得状態ではなく、事業概要、主要事業、IR / 公式資料、主要定量指標、直近イベント、良材料候補、注意材料候補、未確認項目をユーザーが読める形で整理する。

...
- Phase 15 has started with a deterministic `backend/scoring` Investment Score contract and service that combines screening score, forecast agreement, and data quality without changing `ScreeningScore`.
  Phase 15 ? deterministic ? `backend/scoring` Investment Score contract/service ??????`ScreeningScore` ?????????? screening score?forecast agreement?data quality ??????????
...
Phase 15 has started: the first deterministic Investment Score contract exists in `backend/scoring`, and the remaining work is API/UI/export wiring, configurable weights, and richer risk-signal integration.
...
- Improve forecast chart readability with clear actual-vs-forecast styling, labels, and beginner-friendly metrics.
  ?????????????????????????????? forecast chart ??????????
- Wire the `backend/scoring` Investment Score into API/UI/export once the backend contract shape is accepted.
  `backend/scoring` ? Investment Score contract ???????? API/UI/export ?????????

...
| FS-006b | Research Summary / News source URL | ニュースURL表示が未実装に見える | 外部参照ソースと詳細データにはURL表示があるが、`最新ニュース・開示サマリー` 付近から辿りにくい | `最新ニュース・開示サマリー` と `ニュース・開示の出典を表示（URL付きN件）` を `Market Intelligence` / news-feed UI として表示し、URL付き出典がある場合は初期展開して、ニュース、TDnet、企業IR、EDINET、Yahoo Finance をカード全体クリックで開けるようにする。ニュース専用URLが無い場合も、外部参照ソース側に公式資料・provider URLがある可能性を案内する | URL表示未実装の誤解、ニュースと公式開示の混同、provider raw fields の露出、クリック可能性の弱さ | UX readability | High | In review | UI表示限定slice実装済み。href / target / rel を保持し、取得ロジック、外部source正規化、Research Score、Ranking順位、transient-by-default方針は変更しない |
| FS-006c | Research Summary / Investment hint news | 投資ヒントになるニュース専用の見せ方が薄い | ニュース・開示・provider source が出典表示で混ざり、一般ニュースだけを読む導線が弱い。URL文字列や長い説明が多く、ヘッドライン確認として重い | `最新ニュース・開示サマリー` 直後に `投資ヒントとなるニュース` / `注目材料 Top 3` を置き、URL付き `StockNewsEvidence` だけを最大3件カード化する。IR/開示とは別系統の `Market Intelligence` ヘッドラインカードとして、タイトル、公開日、鮮度、出典、材料分類、確認観点、短い要約、種別アクセントを優先し、カード全体クリックでURLへ移動できるようにする。TDnet、企業IR、EDINET、provider profile、URL不足ニュースは混ぜない。Google News RSS を first general-news headline adapter として追加し、取得ニュースは同じカードに流す | ニュースと公式開示の混同、売買推奨やランキング順位材料としての誤読 | UX readability | High | In review | Google News RSS first slice と Market Intelligence Top 3 UX slice 実装済み。paid/API news provider 拡張は後続 scope。Research Score、Investment Score、Ranking順位は変更しない |

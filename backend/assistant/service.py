...
class AssistantResponse(StrictBaseModel):
    ...
    gateway_error_type: str | None = Field(default=None, min_length=1)
    gateway_error_message: str | None = Field(default=None, min_length=1)
    ...
    provider_error_message: str | None = Field(default=None, min_length=1)
...
def _focused_answer(question: str, context_hint: str, section_phrase: str) -> str:
    ...
    answers = {
        "reversal_expectation": (
            f"{section_phrase}の上向き兆候は、下落・調整・横ばいから上向きに変わる兆しを、"
            "チャート形状、予測余地、下落安全性、データ品質に分けて確認する指標です。"
            "上昇気配はすでに出ている上向きの強さを見ます。上向き兆候は、押し目反発、"
            "底打ち接近、横ばい上放れ、蓄積準備の深掘り優先度です。"
            "高スコアでも買い推奨ではなく、下降警戒と下落理由を確認してください。"
        ),
        "news_flow": (

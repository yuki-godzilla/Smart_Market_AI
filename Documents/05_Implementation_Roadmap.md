...
- `投資レーダー` 画面には、肥大化防止に関係する状態を簡潔に表示できるようにする。
  - 通常表示はタイトル右上の `情報鮮度` と取得時刻に絞る。キャッシュサイズや更新履歴は初期画面に常時カード表示しない。

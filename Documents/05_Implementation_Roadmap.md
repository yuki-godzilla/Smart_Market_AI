...
| Phase 30 | 🟨 **MVP済み / 継続拡張** | SMAI Assistant Tool Plan、confirmable navigation、Confirmable Safe Actions、Guided Workflow、optional LLM Tool Planner MVP。`create_decision_report`、`update_research`、`refresh_news` は確認付き実行に接続済み。 |
...
| Phase 32-C / Favorites-aware Refresh | 🟦 **MVP実装済み** | `watchlist` mascot KeyError を修正。favorites refresh metadata、更新状態判定、更新推奨バッジ、最大件数付きローカル更新、更新結果 summary を追加。 |
| Phase 32-C2 / Watchlist UI Polish | 🟦 **MVP実装済み** | 登録済みお気に入りボタンをゴールド系で明確化。Myウォッチリストカードをヘッダー / 状態バッジ / メトリクス / 確認情報へ整理し、専用 `watchlist` mascot asset filename と未配置fallbackを追加。投資レーダーのニュースカード内 `本文に出た銘柄` / `SMAI推測候補` は、銘柄表示とお気に入り操作を1行の横並びチップに統合。 |
| Phase 32-D / Decision Trail | 🟦 **MVP実装済み** | favorites に判断メモ fields (`watch_reason`, `decision_status`, `decision_note`, `next_check_at`, `next_check_label`, `decision_updated_at`, `decision_trail`) を後方互換で追加。Myウォッチリストでカード/テーブル表示、1銘柄ずつ編集、Cockpit/AI調査/レポートへの context handoff を実装。 |
| Phase 32-E / My Radar | 🟦 **MVP実装済み** | 表示専用 Radar priority helper、My Radar summary、今日見る候補 / 注意候補 / 更新候補 / 調査候補 / メモ未入力候補、表示filter/sortを追加。表示順だけを変え、`favorites.json` の保存順や score / Ranking は変更しない。 |
| Phase 32-E2 / My Radar Compact Polish | 🟦 **実装済み** | My Radarを5指標のcompact summaryと判定理由expanderへ整理。更新上限を更新オプションへ移し、主要操作を横並び化。未入力Decision Trailを1行へ畳み、入力済みカードだけ詳細を表示する。 |
...
| Phase N1〜N6 / Notification Platform | 🟦 **N1〜N5-C foundation実装済み / N6第6B slice完了** | schema v5通知履歴、型付きcatalog、設定/dedupe対応Producer、専用通知センター、手動生成、opt-in scheduler/job registry/run log/runnerまで実装。N6第1 sliceは、user-scoped favorites/watchlist snapshotsとfresh News cacheをread-only adapterでschedulerへ接続し、データ不足・stale・対象外はsampleへfallbackせずskip logへ記録する。第2 sliceは明示Cockpit AI調査完了時だけ、report fingerprintによるdedupe済み `smai_analysis_complete` を作成する。第3 sliceはcustom userの明示Assistant Report保存後だけ、sanitized artifact hashでdedupeした `smai_report_ready` を作成する。第4/5 sliceは有効・鮮度内のWatchlist市場計測をFavorite急変通知と日次coverageへ接続する。第6A/B sliceは急変通知を保存済み市場属性、日本・米国のregular session、review済みローカル休場日・短縮取引カレンダーへ限定し、calendar未カバーはfail-closedとする。default user、画面rerun、preview、downloadでは発火しない。 |
...
- 🟦 **N1〜N5-C foundation実装済み / N6第6B slice完了**: 独立gateway、親client/adapter、versioned SQLite設定/履歴、catalog、Producer、専用通知センター、manual generation、opt-in scheduler基盤を実装。N6第1 sliceは実favorite/news/sector cache adapterをschedulerへ接続済みで、第2 sliceは明示Cockpit AI調査完了event、第3 sliceはcustom userの明示Assistant Report保存eventを接続した。第4/5 sliceは鮮度内のWatchlist市場計測をFavorite急変通知と日次coverageへ接続する。第6A/B sliceは保存済み市場属性による日本・米国のregular sessionへ、2026年のreview済み休場日・短縮取引calendarを組み合わせ、calendar未カバーをfail-closedにする。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。
...
| 6 | 🟨 **MVP済み / 継続拡張** | Phase 30-C: Assistant Confirmable Safe Actions | Tool Plan の安全なactionを、ユーザー確認後だけ実行する | Action Execution Layer、`AssistantActionResult`、session-local audit、確認UI、結果カード、`create_decision_report`、`update_research`、`refresh_news` は実装済み。`create_ranking` は後続 | 確認なし実行なし。取得結果は安全な要約だけ表示。順位・スコア・予測値変更、broker操作はしない |
...
- `tools/check_symbol_universe_metadata_coverage.py` produces `data/marketdata/symbol_universe_quality_report.json` as the current network-free DB quality baseline. It reports total/region/product counts plus stock_jp, stock_us, and ETF coverage. Phase 31-SDB keeps unavailable values null, adds optional classification / reliability / ETF fields and per-metric source/as-of/quality metadata, and provides `--fill-missing-only` for explicit provider backfills without replacing existing canonical values.
- `tools/normalize_symbol_universe_quality.py` now backfills missing metric provenance (`*_source`, `*_as_of`, `*_quality`) from existing metadata fields and can optionally mark obvious outliers as `suspicious` without introducing network dependencies.
- `tools/export_symbol_universe_metadata_gaps.py` and `tools/apply_symbol_universe_metadata_patch.py` define the reviewed low-coverage maintenance path: export gap candidates by market/metric preset, fill a reviewed CSV from trusted sources, then apply it while preserving provenance and recording `manual_source_url:*` reasons.
- `tools/run_symbol_universe_metadata_batch.py` and refresh manifests now record `no_update_symbols` / `unchanged_update_symbols` and emit batch `no_update_symbols.csv`, so weak-coverage live runs can distinguish provider-no-data from fill-missing-no-change outcomes during manual review.
...
状態: 🟨 **MVP済み / 継続拡張**。Phase 28-A Cockpit、28-B Ranking、28-C Investment Radar、28-D News screen MVP は実装済み。Research Summary / Decision Report への展開は後続。
...
- 28-D News screen: 実装済み。`news_interpretation.v1`は保存済みNews snapshotを最大8 sectionへ圧縮し、最大4材料について事業・業績へのimpact direction候補と確認horizon、最大4 related sector、鮮度・重複・入力上限外のnoise、最大3 Cockpit handoffを整理する。impact directionは株価方向ではなく、既定disabledかつ明示実行だけである。
...
- 28-D では保存済みNews snapshotだけを使い、既定disabled、明示実行、`user_id + context_hash`分離、custom user別live-only 6時間cache、default user session-only、未知のcontext / material / citation / sector / candidateと助言・株価方向表現の全payload拒否、deterministic fallbackを実装済み。生成とCockpit handoffはNews更新、価格取得、RAG検索を開始しない。
...
- Phase 30-C: AI調査更新、ニュース更新、確認レポート作成、ランキング作成などの safe actions をユーザー確認付きで接続し、action result を表示する。`create_decision_report`、`update_research`、`refresh_news` は接続済み。News更新は既存refresh/cache経路を確認後だけ呼び、成功 / 前回データ利用 / 失敗と、件数 / カテゴリ / JST更新時刻だけを表示する。`create_ranking` は後続接続。
...
- Phase 30-E: Gateway 経由の LLM Tool Planner を optional にし、available actions 限定、schema validation、safety validation、deterministic fallback を必須にする。実装済み。`assistant.llm_planner.enabled=false` が既定で、Gateway `/api/v1/assistant/tool-plan` の JSON 案は親SMAI側で `AssistantToolPlan` / `AssistantGuidedWorkflow` に変換後、unknown action、未確認外部取得、unsafe wording、`create_ranking` などを検証してから既存 UI に採用する。`refresh_news`は確認必須actionとして許可する。
...
### 5.23 🟨 Phase N1〜N4: Notification Platform MVP

目的:

- SMAI 本体から通知 channel を分離し、アプリ内通知と ntfy Push を安全に追加する。
- 将来の Discord / Telegram / Email adapter と HTTP API 化に備える。

導入順:

1. N1: 独立 `smai-notification-gateway`、contracts、NtfyChannel、Dispatcher、単体テスト（実装済み）
2. N2: SMAI 親側 notification client と明示的なテスト通知（実装済み）
3. N3-A: 親 `NotificationRequest` / 子 `NotificationEvent` と配信結果を変換する遅延import adapter（実装済み）
4. N3-B: ntfy ON / OFF、server URL、秘匿 topic、severity threshold、quiet hours、Streamlit設定UI（実装済み）
5. N4: SQLite 通知履歴、未読 / 既読 / archive、固定右上ユーザー入口、プロフィール選択、ユーザー別通知カテゴリ（実装済み。Trusted Device自動選択は廃止）
6. N5: お気に入り、市場、ニュース、SMAI分析のProducerを個別に検証して段階接続
...
- アプリ内履歴を外部送信より先に保存する。
- ntfy 失敗でSMAI本体処理を止めない。
- ntfy topic をログや通常画面へ平文で露出しない。
- `silent`、threshold 未満、quiet hours 中は外部送信しない。
- 初期表示や Streamlit rerun では送信せず、テスト通知は明示操作だけで実行する。
- 通常 checks は fake transport / fixture を使い network-free にする。
- 通知基盤は投資判断、スコア、Ranking、Forecast、Research、broker処理を変更しない。

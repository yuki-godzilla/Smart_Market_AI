...
| FS-006c | Research Summary / Investment hint news | 投資ヒントになるニュース専用の見せ方が薄い | ニュース・開示・provider source が出典表示で混ざり、一般ニュースだけを読む導線が弱い。URL文字列や長い説明が多く、ヘッドライン確認として重い | `最新ニュース・開示サマリー` 直後に `投資ヒントとなるニュース` を置き、URL付き `StockNewsEvidence` だけを最大3件カード化する。IR/開示とは別系統のヘッドラインカードとして、タイトル、公開日、鮮度、出典、材料分類、確認観点、短い要約を優先し、カード全体クリックでURLへ移動できるようにする。TDnet、企業IR、EDINET、provider profile、URL不足ニュースは混ぜない。Google News RSS を first general-news headline adapter として追加し、取得ニュースは同じカードに流す | ニュースと公式開示の混同、売買推奨やランキング順位材料としての誤読 | UX readability | High | In review | Google News RSS first slice とヘッドラインカードUX slice 実装済み。paid/API news provider 拡張は後続 scope。Research Score、Investment Score、Ranking順位は変更しない |

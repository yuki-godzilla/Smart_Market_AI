...
- Research Summary readability first slice: local rule-based `ResearchBriefBuilder` is implemented before any external LLM work. The primary UI order is AI整理メモ -> 読み方サマリー -> 3つの確認ポイント (`会社概要` / `確認できた事実` / `公式資料で未確認`) -> 確認不足 -> 次に確認すべき資料, while source cards, Research Score, external source traces, metric counts, and detail tables are behind expanders. AI整理メモ is a visible card with non-advice / missing-metric badges, quantitative metrics stay compact, and missing metrics are shown as official-source confirmation items. The first metric extraction uses deterministic regex / keyword rules and shows missing metrics explicitly.

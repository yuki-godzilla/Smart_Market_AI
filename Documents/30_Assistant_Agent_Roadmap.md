...
# 上向き兆候 / 本気分析モード / 予測モデル評価に関するAssistant方針

Assistantは、上向き兆候の意味、上昇気配との違い、形状ラベル、配当罠・落ちるナイフ、予測信頼度、モデル見解のばらつき、本気分析モード、LLM性能試験の理由を説明できるようにする。

上向き兆候を買い推奨として断定せず、LLM材料分析だけで順位や投資判断を決めず、ユーザー確認なしに本気分析モードを実行せず、予測を確定的な未来として説明しない。

Confirmable Safe Actions候補:

- 上向き兆候Rankingを開く。
- 上位候補をCockpitで確認する。
- 本気分析モードの説明を表示する。
- 選択銘柄のAI材料分析を実行する。
- Decision Reportへ上向き兆候の観点を追加する。
- 予測モデル評価reportを表示する。

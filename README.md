...
- Phase 23: Optional Adapter / 高度分析を先に進める。Advanced Forecast は `advanced_linear` / `advanced_tree_sklearn` / `advanced_gbdt_sklearn` / `advanced_quantile` の registry、forecast service / API adapter selection、Cockpit `AI予測インサイト` chart/card/detail、Ranking auxiliary 表示、上昇気配 / 下降警戒への控えめブレンド、AI総合への軽量統合まで実装済み。Cockpit の AI予測インサイト初期表示は、結論、統合予測、予測レンジ、予測価格レンジ、信頼度理由、モデル合意度、予測ばらつき、注意点、予測期間に整理し、個別モデル / RMSE / 方向一致率 / 単純予測比較は折りたたみ配下で確認する

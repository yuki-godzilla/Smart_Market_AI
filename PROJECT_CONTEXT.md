...
Phase 14 has Forecast Summary, ensemble forecast, Model Registry Lite, and a forecast-agreement signal connected into Screening Score.
...
- Improve forecast chart readability with clear actual-vs-forecast styling, labels, and beginner-friendly metrics.
  ?????????????????????????????? forecast chart ??????????
- Prepare Phase 15 by defining an investment score contract that can reuse screening and forecast signals.
  Phase 15 ????????? screening ? forecast signal ?????? investment score contract ??????

...
## 現行方針: 個人用・判断を速くするUI / 2026-09-08

この節を現行の優先方針とし、下記の過去の記述と矛盾する場合はこちらを優先します。

SMAI は個人で使う分析ツールです。画面の第一目的は、短時間で「何が良さそうか」「何を警戒するか」「次にどこを見るか」を把握できることです。法務的な注意や一般論を各カードへ繰り返さず、数値・根拠・更新日時・欠損状態を主表示にします。

- 主画面は結論先行にする。見出し、値、短い理由、次の操作を優先し、説明は一文までにする。
- `有望`、`買い候補`、`売却候補`、`警戒`、`注目` は、スコア・予測・条件・根拠が近くにある文脈では使ってよい。`必ず上がる`、`確実`、`安全`、利益保証のような断定は使わない。
- `売買推奨ではありません` は常設の免責文にしない。必要な場合も、レポート、AI生成文、外部データだけを根拠にした画面の補足へ一度だけ置く。
- データ不足・古さ・取得失敗は投資魅力度と混ぜない。状態ラベルは `未取得` より具体的に、`価格未更新`、`資料なし`、`根拠が古い`、`取得失敗` のように表示する。
- 操作ラベルは動詞で始める。例: `ランキング作成`、`資料を読む`、`詳細を見る`、`更新する`。`確認してください` の連続は避ける。
- 既定では詳細を閉じる。計算式、長い読み方、取得トレース、免責、開発者向け情報は `?`、折りたたみ、詳細画面へ置く。
- 同じ説明をサイドバー、ヘッダー、カード、表の直前に重複させない。ページごとに「この画面でできること」は一度だけ示す。
- 判断に使う語は根拠を伴わせる。例: `買い候補` の近くにスコア理由・予測レンジ・データ更新日時を置き、ラベルだけで判断させない。

### 共通用語

- Ranking の上位表示は `注目銘柄`、個別表示は `選択銘柄`、並べ方は `ランキング基準` とする。
- Research の資料数・鮮度・充実度は `根拠資料` とする。通常表示で `Research確認材料` は使わない。
- Cockpit の流れは `総合評価` → `価格・AI予測` → `AI調査・材料分析` → `AIメモ` → `確認レポート` とする。
- Rebalance の計算結果は `売買案` とする。画面では `BUY / SELL` を `買い / 売り` と表示し、`注文は実行しません` を売買案の近くに一度だけ示す。内部contractとCSVの安定keyは変更しない。
- Decision Report の画面名は `確認レポート` に統一する。保存ボタンは `Markdownを保存` のように短くし、形式の用途は共通ガイドとhelpへ置く。

### 表現の基準

| 状況 | 画面で使う表現 | 詳細へ送るもの |
| --- | --- | --- |
| 上位候補 | `注目銘柄`、`買い候補`、`深掘り候補` | スコア算出の全内訳 |
| 下振れ・不確実性 | `警戒`、`要注意`、`レンジが広い` | 一般的なリスク説明 |
| データ状態 | `価格未更新`、`資料なし`、`根拠が古い` | provider / cache / trace の技術詳細 |
| AI要約 | `AIメモ`、`材料の要約` | モデル名、検索方式、raw fields |
| 行動 | `詳しく見る`、`資料を読む`、`売買案を作成` | 毎回の注意書き |

この方針は、投資ロジックを変更するものではありません。表示する順位・スコア・予測値は、従来どおり deterministic な計算結果を正とします。
...
- Ranking は `注目銘柄` を入口にし、ランキング基準、スコア内訳、リスク、データ信頼度を近くに表示する。
...
- Rebalance は `現在配分`、`目標配分`、`売買案`、`リスク判定` の順で表示する。売買案は計算結果として明確に見せ、注文を実行しないことを近くに示す。
...
- Forecastで役割別モデル選択を表示する場合、価格シナリオは`中心予測`、上下判定用の別headは
  `方向判定用変化率`、price centerだけに使わなかったadapterは`中心値から除外`と表現する。
  `除外モデル`だけで個別予測自体が無効・非表示になったように見せない。`モデル選択`には期間帯、
  center model数、選択理由を併記し、60日超は`監査外`と`信頼度 低め`を近くに示す。
- Forecast confidenceは役割を曖昧にまとめず、`中心予測の信頼度`と`方向判定の信頼度`を分ける。
  60日超では中心を低めに維持し、120日以下の方向だけが内部検証に応じて中くらいになり得ることを
  helpで説明する。方向信頼度だけでRanking寄与が増えると誤解させない。
...
SMAIアシスタントで明示的にニュース更新を依頼された場合は、回答を
`ニュースを更新できます。内容を確認してから実行してください。`にそろえ、
操作ボタンは`ニュースを更新する前に確認`、確認後は`ニュースを更新`とする。
キャンセル結果は`ニュース更新をキャンセルしました`とし、外部取得・保存を行っていないことを示す。
`ニュース材料を見たい`などの閲覧・相談表現を更新操作へ読み替えない。

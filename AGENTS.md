...
- `PROJECT_CONTEXT.md`: current state and work log / 現在地と作業ログ
...
| New phase work | roadmap current phase | related service + tests + project context | broad refactor |
...
If docs and code disagree, trust code for current behavior and record the mismatch in `PROJECT_CONTEXT.md` when material.
ドキュメントとコードがズレる場合、現在挙動はコードを優先し、重要な差分だけ `PROJECT_CONTEXT.md` に記録します。
...
- keep `AGENTS.md` stable; move volatile status/logs to `PROJECT_CONTEXT.md`
...
Read `PROJECT_CONTEXT.md` when implementation status, assumptions, work-log, or code/doc mismatch matters.
実装状況、前提、作業ログ、コード/文書差分が重要な場合に読む。

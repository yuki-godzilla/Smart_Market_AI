...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UX-001 | Symbol Cockpit | 1銘柄を初めて確認する | 銘柄を選び、期間を選択し、価格データを取得する | 価格、特徴量、Forecast、Investment Score、注意点の関係が順番に理解できる | 情報量が多く、最初に何を見るべきか分からない | UX Confusion | High | Resolved | 2026-09-08 F2で取得後主導線を`01 総合評価`→`02 価格・AI予測`→`03 AI調査・材料分析`→`04 AIメモ`→`05 確認レポート`へ統一。mock 7203.Tを4 viewportで再確認。live provider確認は任意の運用smokeとして継続 |
...
| UX-003 | Symbol Cockpit | データ不足時の確認 | 欠損や provider warning がある銘柄を表示する | 何が不足し、判断時にどう注意するか分かる | 欠損が単なるエラーに見える、または見逃される | Data Confidence | High | In review | 2026-07-16にCSVで未対応のVを取得し、`価格データを取得できませんでした`、`APP-2000: Unsupported csv symbol`、設定・銘柄・期間の再確認導線を実画面確認。Yahooへ戻した再取得も成功。低DB信頼度の詳細表示は継続確認 |
| UX-004 | Ranking | 候補探索を開始する | 条件を選択し、候補数を確認してランキング作成する | Ranking が候補探索・比較の入口だと分かる | 上位銘柄が推奨銘柄に見える | Product Safety | High | In review | 2026-09-08 F2で主表示を`銘柄ランキング` / `ランキング基準` / `注目銘柄` / `選択銘柄のスコア内訳`へ短縮し、`根拠資料`、リスク、データ信頼度の用語を統一。mockの4 viewportで条件・履歴導線を確認。live上位結果は継続確認 |
...
| UX-009 | Rebalance Cockpit | 保有資産の配分を見直す | sample または手入力で目標配分を設定し、結果を確認する | 現在配分、目標配分、売買案、リスクの順で読める | 提案売買が注文指示に見える | Product Safety | High | Resolved | 2026-09-08 F2で`売買案を作成`、`買い / 売り`表示へ統一し、売買案の近くに`注文は実行しません`を一度表示。mock結果を4 viewportで確認 |
...
| UX-013 | Decision Report | Rebalance report を読む | Rebalance 結果から report を作成する | 現在配分、目標配分、売買案、リスクの記録として読める | 売買指示書に見える | Product Safety | High | Resolved | 2026-09-08 F2で結果確認後にレポートを置き、画面名を`確認レポート`へ統一。保存ボタンを短縮し、用途は共通ガイドとhelpに分離 |
...
| UX-019 | Market Data provider / data freshness | Yahoo live provider を使う | live provider で取得、失敗、再取得を確認する | provider、対象期間、取得失敗、鮮度が分かる | 古い/欠損データであることを見落とす | Data Confidence | High | In review | 2026-07-16にYahooで7203.TとVを取得し、取得元・基準日・期間・商品/地域を確認。CSV失敗後にYahooで再取得できた。stale表示の時間経過確認は継続 |
| UX-020 | Market Data provider / data freshness | 大量ランキング取得を行う | 30件超の live ranking を実行する | 時間がかかること、部分失敗があり得ることが分かる | 途中停止や部分結果の意味が分からない | Error Recovery | Medium | Resolved | 2026-07-16にYahoo上位100件を実作成。4%→19%の処理進捗を表示し、投資レーダーへ遷移後もサーバー側ジョブが継続、再表示で99件と価格未取得1件を明示した |
...
| UX-023 | Navigation / Flow | Ranking から Cockpit へ移動する | Rankingで選択銘柄をCockpitに渡す | 比較から深掘りへ進む導線だと分かる | 画面遷移後に何が引き継がれたか分からない | State Visibility | Medium | Resolved | 2026-07-16に100件Yahoo RankingのVisaからCockpitへ遷移。対象銘柄、Yahoo、2026-04-17〜2026-07-16を明示して引継ぎ、再取得は自動実行せずユーザー操作へ留めることを確認 |
| UX-024 | Error Recovery | provider failure から復帰する | provider error 後に期間や銘柄を変えて再実行する | 原因と次の行動が分かる | raw error が難しい、再試行方法が不明 | Error Recovery | High | Resolved | 2026-07-16にCSVの未対応Vで失敗表示を確認後、Yahooへ切り替えて同一銘柄の取得成功まで確認。画面はdomain error codeと次の確認を示し、stack traceは表示しない |
| UX-025 | Investment News dashboard | 投資レーダー画面を開く | サイドメニューから `投資レーダー` を開き、市場ニュースヘッドライン、投資ヒートマップ、カテゴリ別ニュースレーン、銘柄名付き関連銘柄導線を確認する | 市場ニュースの温度感と確認候補を把握し、気になる材料は `銘柄コックピット` で深掘りする入口だと分かる。Investment Score / Research Score / Ranking順位は変わらない | ニュースや投資ヒートマップが売買推奨や銘柄推奨に見える、値動き色が投資判断そのものに見える | Product Safety | High | Resolved | 2026-07-16にlive cacheで実測表示を確認。2026-09-08にnetwork-free mockで価格取得不能時も擬似タイルを出さず、理由・再確認方法・非順位境界を5 viewportで確認。ニュース一覧の確認候補からCockpitへの導線も再確認 |
...
| UX-N04 | 通知 / 右上ユーザーエリア | 全画面から表示ユーザーと設定入口を確認する | 固定右上入口から通知センター、ユーザー設定、通知設定、ユーザー切替を操作する | 通知確認と設定の役割が分かれ、固定入口がスクロールや画面外sidebar状態で消えない | スマホで横overflow、スクロールで入口が消える、設定の役割が混ざる | Security / Responsive UX | High | Resolved | 2026-09-08にiPhone 375、iPad 810、PC 1366でプロフィール選択、固定表示、scroll追従、通知センター遷移、横overflowなしを確認。iPadで画面外sidebar状態により入口が隠れる問題を修正 |

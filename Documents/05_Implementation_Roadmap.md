...
| Phase N1〜N4 / Notification Platform | 🟨 **N1/N2/N3-A実装済み / N3-B/N4未着手** | 独立gateway、親client、親子adapter、明示テスト通知関数を実装。設定保存/UI、既存イベント接続、SQLiteアプリ内履歴、通知センターは未実装。 |
...
- 🟨 **N1/N2/N3-A実装済み / N3-B/N4未着手**: Phase N1〜N4 通知基盤。独立gateway、親側client境界、親子contract adapter、明示テスト通知関数まで実装済み。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。設定保存/UI、既存イベント接続、履歴UIはまだ行わない。

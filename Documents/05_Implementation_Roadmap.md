...
| Phase 30 | 🟨 **MVP済み / 継続拡張** | SMAI Assistant Tool Plan、confirmable navigation、Confirmable Safe Actions first MVP。`create_decision_report` だけ確認付き実行に接続済み。 |
...
| 6 | 🟨 **MVP済み / 継続拡張** | Phase 30-C: Assistant Confirmable Safe Actions | Tool Plan の安全なactionを、ユーザー確認後だけ実行する | Action Execution Layer、`AssistantActionResult`、session-local audit、確認UI、結果カード、`create_decision_report` first MVP は実装済み。`update_research` / `refresh_news` / `create_ranking` は後続 | 確認なし実行なし。外部取得、順位・スコア・予測値変更、broker操作はしない |
| 7 | ⬜ **後続 / 未着手** | Phase 30-D/E: Guided Workflow / Optional LLM Tool Planner | Ranking -> Cockpit -> Research -> Report の確認付きworkflowと、限定action catalog内のLLM plannerを検討する | multi-step state、action result chaining、schema validation、deterministic fallback、evaluation harness | 完全自律化しない。unsafe / hallucinated action を拒否する |
...
状態: 🟨 **Phase 30-A / 30-B / 30-C first MVP済み / 継続拡張**。
...
- Phase 30-C: AI調査更新、ニュース更新、確認レポート作成、ランキング作成などの safe actions をユーザー確認付きで接続し、action result を表示する。first MVPでは `create_decision_report` だけを接続済み。Action Execution Layer、`AssistantActionResult`、session-local audit、確認UI、結果カードを追加し、成功時は既存Decision Report下書きプレビュー / 保存導線に接続する。`update_research` / `refresh_news` / `create_ranking` は後続接続。
...
- 30-C first MVP: `create_decision_report` は確認パネルで対象、使用材料、外部取得なし、変更しないものを示してから実行する。実行結果はsuccess / failed / skipped / cancelled / not_availableを区別してchat threadに残し、最小限のaudit情報をsession-localに保持する。外部取得、ランキング作成、スコア・予測値変更、broker操作は行わない。

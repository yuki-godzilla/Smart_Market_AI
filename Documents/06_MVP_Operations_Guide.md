...
- Advanced Forecast opt-in ranking sort profile: Ranking already retains and displays `advanced_linear` auxiliary fields, but using them to change ranking order remains future opt-in scope
...
- Ranking rows can include `advanced_linear` auxiliary forecast fields (`predicted_return_5d`, `predicted_return_20d`, `advanced_forecast_score`, `advanced_forecast_confidence`). These are shown as `高度予測5日` / `高度予測20日` / `高度予測スコア` / confidence context in the ranking table and selected-candidate details when enough local history exists. They do not change ranking order by default.

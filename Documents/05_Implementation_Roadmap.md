...
Research RAG は Phase 20 local evidence slice が deterministic foundation として実装完了です。今後の product direction は、local 登録資料を主データ源にするのではなく、`AI調査を更新` で外部の最新IR・開示・ニュース・provider 情報を取得/参照し、session-local に一時分析する流れを基本にします。local 登録資料は通常 tests / demo seed / archive / fallback の位置づけです。Phase 21 高度Research RAG（根拠抽出・根拠付き回答生成）は query expansion、structured extraction、grounded answer、retrieval quality、evidence reranker、UI / Decision Report 表示、optional vector / hybrid contract と scoring、keyword-fallback hybrid retrieval wrapper、local embedding generation、optional vector-index build workflow、in-memory local vector store、file-backed vector cache、local `ResearchBriefBuilder` readability layer の初期 slice が進行中です。`ResearchFactSummary` の初期 slice も実装済みで、事業概要、主要事業、地域・収益源、主要IR/公式資料、主要定量指標、業績見通し、配当・株主還元方針、直近イベント、良材料候補、注意材料候補、未確認項目を source-backed fact として UI に出します。さらに `CompanyResearchSummaryBuilder` は `CompanyResearchEvidence` 正規化層を通して source type ごとの役割を分け、企業概要、主な事業、製品・サービス、地域、規模感、定量情報、IR情報、最新ニュース・開示へ再分類します。IR / news / metric は found / missing / unparsed / unverified を区別し、AI読み取りメモは初期表示ではなく詳細側に下げます。display-only の `InvestmentInsightBuilder` と `InvestmentQuestionSummaryBuilder` は後段の補助メモとして残し、Research Score、Investment Score、ranking order には反映しません。主表示では `企業リサーチサマリー`、`定量情報サマリー`、`IR情報サマリー`、`最新ニュース・開示サマリー` を先に出し、出典カード、Research Score、件数、confidence、外部取得失敗の技術詳細は詳細側に下げます。EDINET optional metadata/link adapter + TDnet + Yahoo Finance の外部取得初期 slice は実装済みです。Phase 22 Research Score は backend deterministic service、disabled-by-default Investment Score optional input、Cockpit / Ranking Research Summary display、selected-candidate breakdown context、Cockpit Decision Report section の初期 slice が開始済みです。Ranking order 統合、企業IR site adapter、Assistant、distribution readiness は後続 planned / future scope です。
...
- Research RAG の `ResearchFactSummary` 抽出対象拡張、追加 external source adapter、Research Score の ranking 統合
...
- `EDINETResearchAdapter` provides optional official securities-report metadata/source links when `EDINET_API_KEY` is configured and returns no payloads without the key. `TDnetResearchAdapter` provides official timely-disclosure source links for Japanese listed-company IR, and `YahooFinanceResearchAdapter` continues to provide provider profile and recent news payloads. `ExternalStockNewsFetchService` can also normalize external news adapter output directly into `StockNewsEvidence`, with URL dedupe, freshness warnings, and an explicit network opt-in gate. Tests inject fake HTTP / JSON / ticker/news factories, so no live EDINET, TDnet, Yahoo, or news-provider call is required in normal checks.
...
- R10.5: InvestmentInsight / 投資判断サマリー。初期 slice は実装済み。`ResearchBrief` とは別の display-only contract と builder を追加し、`CompanyResearchReport`、`StockNewsReport`、`ExternalResearchFetchResult` を status、confidence、primary action、良い材料、注意材料、公式資料・主要指標の確認不足に再分類する。Research Score、Investment Score、ranking order には反映しない。
...
- Cockpit Research Summary の主表示順は、企業リサーチサマリー -> 定量情報サマリー -> IR情報サマリー -> 最新ニュース・開示サマリー -> 企業理解の確認ポイント -> folded AI読み取りメモ / 3カラム確認情報（確認できた情報 / 注意して読む情報 / 不足している情報）-> folded 根拠確認（AI整理メモ / 読み方サマリー / 会社概要 / 確認できた事実 / 公式資料で未確認）とする。出典カード、Research Score、件数、confidence、外部参照ソース、取得失敗の技術詳細、詳細データは必要時に開く補助情報へ下げる。
...
- R8: External Source Adapter。EDINET optional metadata/link adapter、TDnet、外部ニュース adapter/service 正規化は初期 slice 実装済み。IR site / additional news providers / RSS などを `AI調査を更新` の標準 source adapter として広げ、通常 checks は fake adapter / fixture で network 非依存にする。

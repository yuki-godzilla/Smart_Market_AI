...
- Research Summary readability first slice: local rule-based `ResearchBriefBuilder` is implemented before any external LLM work. The UI order is AI整理メモ -> 定量評価サマリー -> 企業概要・事業内容 -> 良材料候補 -> 注意材料候補 -> 未確認・不足根拠 -> 次に確認すべき資料 -> 出典カード -> Research Score -> 詳細データ. AI整理メモ is now a visible card with non-advice / source-confidence / missing-metric badges, quantitative metrics are small cards, and missing metrics are shown in a warning panel. The first metric extraction uses deterministic regex / keyword rules and shows missing metrics explicitly.

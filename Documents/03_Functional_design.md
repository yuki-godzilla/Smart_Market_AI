...
- ユーザー向けの `AI調査を更新` では、TDnet timely disclosure、EDINET / 企業IR adapter（後続）、provider profile、news などの外部最新 source を優先する。
- Research Summary は local rule-based `ResearchBrief` で、AI整理メモ、定量指標、企業概要、良材料候補、注意材料候補、不足根拠、次の確認資料、出典カード、詳細データに分けて表示する。
- 次の planned slice では `ResearchFactSummary` を追加し、取得状態ではなく、事業概要、主要事業、確認済みIR / 公式資料、主要定量指標、直近イベント、良材料候補、注意材料候補、未確認項目を source-backed fact として `ResearchBrief` に渡す。
- 外部LLM要約は future / optional とし、次の読みやすさ改善 slice では必須にしない。
...
- 実装済み土台: ローカルPDF/Markdown/Textの登録、chunk化、keyword search、テンプレート要約、query expansion、structured extraction、grounded answer、Research Score 初期 slice、TDnet + Yahoo Finance 外部取得の初期 slice。
- 将来拡張: EDINET / 企業IR / IR RSS / News adapter、ResearchFactSummary fact layer、ranking order の明示 opt-in 統合、optional LLM summary。

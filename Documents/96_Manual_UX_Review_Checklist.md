...
| UX-014a | Research Summary / Brief | ローカルAI整理メモを読む | `AI調査を更新` 後に Research Summary 冒頭を確認する | AI整理メモ、定量評価、企業概要、良材料候補、注意材料候補、不足根拠、次の確認資料が順に読め、provider raw fields が通常表示に出ない | 生データ羅列で何を確認すべきか分からない、またはAIが投資判断したように見える | UX Readability | High | Planned | 外部LLMなしの local rule-based ResearchBrief を前提に確認 |

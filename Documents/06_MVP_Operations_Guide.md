...
Investment News dashboard は Phase 22.x の初期MVPとして、サイドメニューの `投資レーダー` から開けます。現時点では `backend/news` の snapshot / status / cache / refresh manager と deterministic dashboard builder を使い、保存済みsnapshotがなければ fake snapshot / fixture から市場ニュースヘッドライン、株式ヒートマップ風の投資ヒートマップ、3列のカテゴリ別ニュースカード、銘柄名付き関連銘柄の `銘柄コックピット` 導線を表示します。投資ヒートマップは市場指標がある場合は値動き / 取引量を使い、欠ける場合はニュース材料から代理シグナルを補完します。Investment Score / Research Score / Ranking order は変更しません。外部ニュースsource接続、詳細フィルタ、Watchlist連動、通知は後続範囲です。

...
- Phase 30-H loading polishとして、親SMAI側にInvestment Radar icon、2秒fragment監視、ready / fallback auto transition、入力・履歴保持回帰を追加。Gateway側のendpoint追加は行わない
- Phase 30-H recovery / dynamic model sliceとして、親SMAI側にbounded auto retry、automatic recovery、main-area modal、environment-only model selectboxを追加。Gateway `/models` は後方互換のままmodel metadataを拡張
...
- 親SMAI側 Phase 28-C2 で明示実行型の Investment Radar Overviewを実装済み。Gatewayは`task_type=radar_overview_interpretation` / `response_schema=radar_overview_interpretation.v1`でcontext hash、sector/theme/candidate関係、citation、stale market policyを固定する

...
class NewsInterpretationConfig(BaseModel):
    """Optional Gateway settings for explicit News screen interpretation."""
    ...
    prompt_version: str = Field(default="news_interpretation_mvp.v1", min_length=1)
    schema_version: str = Field(default="news_interpretation.v1", min_length=1)
    cache_enabled: bool = True
    cache_ttl_seconds: int = Field(default=21600, gt=0, le=86400)
    max_material_groups: int = Field(default=4, gt=0, le=4)
    max_evidence_per_group: int = Field(default=3, gt=0, le=3)
    max_sector_groups: int = Field(default=4, gt=0, le=4)
    max_handoff_candidates: int = Field(default=3, gt=0, le=3)

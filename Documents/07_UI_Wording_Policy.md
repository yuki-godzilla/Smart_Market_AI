...
ニュースURL表示は `外部参照ソース` と詳細データに実装済みです。`最新ニュース・開示サマリー` 周辺では、URL付きニュース・開示がある場合は `ニュース・開示の出典を表示（URL付きN件）` を初期展開して確認できるようにし、ニュース専用URLが無い場合も外部参照ソース側に公式資料・provider URLがある可能性を案内します。ニュース/開示領域は `Market Intelligence` として通常カードから分け、URL付き項目はカード全体クリックで開ける news-feed UI にします。
`投資ヒントとなるニュース` は、URL付きの一般ニュースだけを切り出すカードです。Google News RSS や Yahoo Finance news 由来の一般ニュースを対象にし、TDnet、企業IR、EDINET、Yahoo Finance profile などの source trace と混ぜません。IR/開示の根拠カードとは別系統の `注目材料 Top 3` ヘッドラインUIにし、タイトル、公開日、鮮度、出典、材料分類、確認観点、短い要約、種別アクセントを優先します。raw URLは本文表示せず、カード全体をクリックして外部URLを開く導線にします。href、`target="_blank"`、`rel="noopener noreferrer"` は維持します。

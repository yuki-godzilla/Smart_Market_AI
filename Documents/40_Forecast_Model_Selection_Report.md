...
1. `horizon-conditioned conservative calibration`のevaluation-only実装、固定profile化、
   新規60symbolでの再現評価は完了。
2. 直近再現は通過したが、2021年末履歴再現のETF・60日が重大劣化gateに抵触した。runtimeへ
   接続せず、今回の監査結果に合わせた再調整もしない。
3. 実ニュース・IRのpoint-in-time LLM factor archiveを作る。raw本文は保存方針に従い、source traceと
   structured factorを保存する。
4. LLMはconfidence / range調整からshadow評価し、数値returnへ直接混ぜない。
5. 保守的price headは後日の新暦期間で再監査する。全体とmarket / asset type / regimeのgateを

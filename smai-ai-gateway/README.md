...
親SMAIのloading UIはreadinessを2秒間隔でprocess-local監視し、ready / failed / timeout検知後に通常チャットまたは利用可能なfallback画面へ自動遷移します。このUI再描画と入力・履歴保持も親側の責務で、Gateway API契約は変わりません。
...
`task_type=ranking_interpretation` と `response_schema=ranking_interpretation.v1` は Phase 28-B の明示実行専用です。Gateway は、親SMAIが固定したRanking条件・上位候補順・主要指標・候補集合内sector contextだけを使い、項目別の`cited_evidence_ids`と候補IDを持つstrict JSONを返します。候補順、順位、score、Forecastの再計算や変更は行いません。候補順・citationが不正な応答はGatewayでfallbackとなり、親SMAIも候補・数値・日付・助言表現を再検証します。

`task_type=radar_overview_interpretation` と `response_schema=radar_overview_interpretation.v1` は Phase 28-C2 の明示実行専用です。Gateway は最大8 sectionの固定済みRadar overviewから、取得済み候補集合の値動き、sector、theme、既存deep-dive候補をcitation付きstrict JSONへ整理します。stale / missing marketではmovementをnullとし、direct mention / inferred candidate / macro proxyを混同せず、市場全体への一般化、候補追加、順位・価格・score・Forecast変更を行いません。親SMAIがcontext hash、sector / theme / candidate関係、symbol、数値、日付、助言表現を再検証します。

`task_type=news_interpretation` と `response_schema=news_interpretation.v1` は Phase 28-D の明示実行専用です。Gatewayは最大8 sectionの固定済みNews contextから、事業・業績へのimpact direction候補、確認horizon、related sector、noise、固定済みCockpit handoffをcitation付きstrict JSONへ整理します。株価方向、候補追加、News更新、価格取得、RAG検索、順位・score・Forecast変更は行いません。
...
`/api/v1/assistant/tool-plan` は optional LLM Tool Planner 用の構造化 JSON endpoint です。SMAI 親側が `assistant_tool_plan` request と available actions を渡し、Gateway は `tool_plan` または `guided_workflow` の案を返します。Gateway は action を実行せず、SMAI module も import しません。親SMAI側は unknown action、未確認外部取得、unsafe wording、`create_ranking` などを検証し、採用できない場合は deterministic Tool Plan / Guided Workflow に戻します。`refresh_news` は確認必須の外部取得として提案できますが、実行とNews cache更新は親SMAIだけが担当します。親SMAI側 Phase 30-F では、この planner 出力と fallback 経路を fixture-based Agent Evaluation Harness で network-free に回帰確認します。Phase 30-G1/G2 の Workflow Session / runtime state machine / recovery UI controls も親SMAI側の責務であり、Gateway は引き続き JSON proposal のみを返します。

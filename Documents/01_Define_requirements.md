...
- 既定の `yahoo` live provider 経路と、テスト / オフライン確認用の deterministic `mock` / `csv` provider
...
- Research RAG の company IR site 追加 adapter、EDINET adapter の本文/詳細抽出拡張、ResearchFactSummary 抽出対象の拡張、ranking order 統合
...
Phase 9 までの MVP では、Portfolio-to-Risk workflow、local reporting、外部 MarketData provider の adapter 基盤までを整備しました。
...
- 外部 provider から取得した市場データをユーザー向けの既定経路として利用できること。
- `mock` / `csv` provider による deterministic なローカル検証経路を、テスト / オフライン用の明示設定として維持すること。
...
## 9. 上向き兆候・予測モデル・本気分析モード要件

### 上向き兆候ランキング

旧「反転期待」はユーザー向けには「上向き兆候」とする。下落→上昇、調整→再上昇、横ばい→上放れ、安定→上昇準備を探す独立探索軸であり、買い推奨ではなく深掘り確認の優先度である。落ちるナイフ、長期低迷、高配当罠、上昇済み、材料不足、データ不足を警戒する。

上昇気配は「すでに上向きの強さが出ている銘柄」、上向き兆候は「まだ大きく上がっていないが上向きに変わる兆しがある銘柄」と区別する。

### 既存予測モデル高性能化

`advanced_linear`、`advanced_tree_sklearn`、`advanced_gbdt_sklearn`、`advanced_quantile`、forecast consensusを、direction accuracy、RMSE/MAE、horizon、市場、asset type、regime、上向き兆候への貢献、false positive削減、model disagreement、confidence calibrationで継続評価する。新規モデルの無秩序な追加より、walk-forward評価、特徴量、既存weightingの改善を優先する。

### 本気分析モード

ランキング作成時に任意で「本気分析モード（AI材料分析つき）」を有効化できる。通常の数値ランキング完成後、上位候補だけをAI材料分析し、初期段階では材料バッジ、警告、要約に留める。通常ランキングはLLMなしで高速・安定に完走する。

LLMを順位へ融合する前に、false positive削減、悪材料・配当罠検出、説明品質、latency、failureを比較する性能評価試験を必須とする。有効性が確認できた場合だけ上位候補内で補正幅を限定する。全銘柄リアルタイムLLM分析、売買推奨、買い時・底打ち確定の判定には使わない。

...
- Assistant startup warmup、retry / recovery、readiness state、loading modal、Investment Radar cache headlinesは親SMAI側の責務とする。Gatewayは既存 `/models` に汎用model metadataを提供し、新しいSMAI専用endpointやSMAI Python importは追加しない
- readiness polling、ready/fallbackへのguarded auto transition、入力・chat history・workflow session保持、loading icon fallbackも親SMAI UI側で扱う
...
- Investment Radar候補単位の`radar_interpretation.v1`、画面全体の`radar_overview_interpretation.v1`、Rankingの`ranking_interpretation.v1`は、candidate/context IDと項目別`cited_evidence_ids`を持つstrict JSONをGatewayから返す。Radar Overviewではcontext hash、sector/theme/candidate関係、stale marketのmovement禁止を、Rankingでは固定済みcandidate順を照合する。Gatewayは許可済みcontext IDだけを参照し、親SMAIがsymbol・数値・日付・助言表現のgrounding validationとdeterministic fallbackを担う
...
- `ranking_interpretation` は `context-answer` の明示実行taskとして扱う。Gatewayは最大8sectionの固定済みRanking contextからstrict schemaを返し、親SMAIが追加grounding、live-only atomic cache、user/context別UI state、deterministic fallbackを担う
- `radar_overview_interpretation` は `context-answer` の明示実行taskとして扱う。Gatewayは最大8sectionの固定済みRadar contextからstrict schemaを返し、候補順・価格・scoreを変えない。親SMAIが市場snapshot鮮度、追加grounding、custom-profile live-only cache、user/context別UI state、deterministic fallbackを担う
- `/api/v1/assistant/tool-plan` は optional LLM Tool Planner 用の汎用 JSON 提案 endpoint として扱う。Gateway は available actions を読んで plan 案を返すだけで、action実行、外部取得、Ranking作成、確認レポート作成、SMAI状態更新は行わない。親SMAI側が schema / allowlist / safety validation と deterministic fallback、既存 `次にできること` / `確認フロー` UI採用を担う
...
- Assistant Tool Plan / Guided Workflow の validation / fallback / action execution

...
ニュースURL表示は `外部参照ソース` と詳細データに実装済みです。`最新ニュース・開示サマリー` 周辺では、URL付きニュース・開示がある場合は `ニュース・開示の出典を表示（URL付きN件）` から確認できるようにし、ニュース専用URLが無い場合も外部参照ソース側に公式資料・provider URLがある可能性を案内します。

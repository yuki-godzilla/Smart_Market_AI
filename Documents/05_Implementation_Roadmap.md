...
- Cockpit Decision Report では Research Score が売買推奨ではなく確認材料として表示される。Ranking 表示への自然な接続は後続で扱う。

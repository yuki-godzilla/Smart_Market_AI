...
| FS-006c | Research Summary / Investment hint news | 投資ヒントになるニュース専用の見せ方が薄い | ニュース・開示・provider source が出典表示で混ざり、一般ニュースだけを読む導線が弱い | `最新ニュース・開示サマリー` 直後に `投資ヒントとなるニュース` を置き、URL付き `StockNewsEvidence` だけを最大3件カード化する。材料分類、確認観点、鮮度、要約、なぜ見るか、追加確認、ニュースURLを表示し、TDnet、企業IR、EDINET、provider profile、URL不足ニュースは混ぜない | ニュースと公式開示の混同、売買推奨やランキング順位材料としての誤読 | UX readability | High | In review | UI表示限定slice実装済み。一般ニュースprovider拡張は後続 scope。Research Score、Investment Score、Ranking順位は変更しない |

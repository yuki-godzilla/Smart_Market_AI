...
10. SBI / NISA / future 投信 metadata source import. 部分完了。`--source-profile`、JPX stock/ETF profile、SBI US stock/ETF seed、NISA eligibility update profile/seed、mutual fund seed、投信 metadata columns の import path は追加済み。MVP ranking は stock / ETF のみ対象にし、mutual fund seed は future extension として保持する。次は公式/curated source の拡張。

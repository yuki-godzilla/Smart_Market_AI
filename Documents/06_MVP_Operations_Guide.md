...
- Phase 23 Advanced Forecast adapter registry + `advanced_linear` / `advanced_tree_sklearn` / `advanced_gbdt_sklearn` / `advanced_quantile` backend + API + Cockpit + Ranking auxiliary display slice
  ...
  - Cockpit overlays advanced forecast context on the existing price / forecast chart using the same period-derived horizon as baseline forecasts. The default horizon is roughly one twelfth of the displayed period and capped at 60 days. The initial chart emphasizes actual price, `AI予測インサイト` as the consensus line, and its lower-to-upper prediction range band; individual advanced lines such as `高度予測: ツリーモデル`, `高度予測: ブースティングモデル`, and `高度予測: レンジモデル` remain available for detail confirmation instead of crowding the first view. Naive / moving-average / momentum simple forecasts stay available as backend baseline / detail context, but are not part of the default Cockpit chart or main model-card display. The standard Altair legend sits below the chart, the right forecast-focus chart names the forecast horizon, and the full chart restores small point markers while keeping the actual-price line thinner.
...
- 価格・予測チャート: 初期表示では実績価格、`AI予測インサイト`、予測レンジ帯を先に確認し、個別モデル線の重なりで読みづらくしない。十分な履歴がある場合は `advanced_linear`、`advanced_tree_sklearn`、`advanced_gbdt_sklearn`、`advanced_quantile` を取得期間から決まる共通の予測日数で計算するが、個別高度予測線は詳細確認用として初期OFFにする。`AI予測インサイト` カードは統合予測、弱気 / 中央値 / 強気レンジ、予測価格レンジ、予測期間、信頼度理由、モデル合意度、予測ばらつき、主な理由、注意点を主表示にする。平均 RMSE、誤差改善、過去検証の方向一致率、相対的に安定したモデル、個別モデル結果、単純予測比較は `高度予測モデルの詳細を見る` / `検証指標を見る` / `単純予測との比較を見る` で確認する。Consensus helper には `統合予測 = Σ(各モデルの予測変化率 × 重み) ÷ Σ重み` と、重みを信頼度・誤差改善・モデル合意度・検証数から保守的に丸めることを明記する。各モデルの helper も直近値維持、移動平均、モメンタム、線形、ツリー、ブースティング、レンジの考え方を初心者向けの短い計算式で説明する。予測日数の初期値は取得期間のおよそ 1/12 を使い、60日を上限にする。全体チャートの右側に、最新実績の数日前から予測部分までを自動抽出した拡大図を並べ、タイトルは `今後31日の予測拡大` のように期間を示す。色見本はチャート下部に置き、全体チャートは小さな点マーカーを復活させつつ線を主役にする。チャート系列の強調確認は Streamlit checkbox ではなくチャート上の線 / 点クリックで扱い、補助テーブルで予測変化率、レンジ、信頼度、検証指標、注意点を確認する。naive / moving-average / momentum の単純予測は backend baseline / fallback / 詳細確認用として残すが、既定のチャート表示と主要モデルカードには出さない。
...
- Cockpit price / forecast display leads with `AI予測インサイト`. The card shows a short conclusion, integrated prediction, weak / median / strong return range, forecast price range, confidence reason, model agreement, forecast dispersion, main reasons, cautions, and the forecast horizon. RMSE, error improvement, historical direction accuracy, relatively stable model, individual advanced model cards, and simple forecast baseline comparisons are folded under the chart so the first view stays focused on the integrated forecast and uncertainty. The forecast remains decision-support context, not a future guarantee.

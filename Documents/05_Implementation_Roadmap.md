...
Research RAG は Phase 20 local evidence slice が implementation complete です。Phase 21 高度Research RAG（根拠抽出・根拠付き回答生成）は query expansion、structured extraction、grounded answer、retrieval quality、evidence reranker、UI / Decision Report 表示、optional vector / hybrid contract と scoring の first slice が進行中です。Research Score integration、外部 source adapter、Assistant、distribution readiness は後続 planned / future scope です。

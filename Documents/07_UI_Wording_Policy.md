...
投資レーダーの既定タブは `市場レーダー` とし、`今日のレーダー` と独立した `市場ヒートマップ` に分けません。ヒートマップは `セクター` / `業種` / `注目ニュース` の分類を切り替え、関連銘柄を実価格タイルとして表示します。タイルには企業名とシンボル、正確な騰落率、`本文` / `テーマ推測`、根拠件数を併記し、企業名を主表示、シンボルを補助表示として扱います。タイルクリックは該当銘柄の `銘柄コックピット` に移動する導線とし、ニュース画面だけで判断を閉じない流れにします。面積は絶対騰落率、色は上昇・下落方向とし、期間、取得元、価格基準日時、最低表示面積を凡例へ併記します。期間は `20営業日（約1か月）` に固定して選択UIを置かず、手動操作は `今すぐ更新` とします。ニュース根拠量や材料種別を、`材料+1.2` のような符号付き補完値、実測のような値動き・取引量、好悪色へ変換してはなりません。

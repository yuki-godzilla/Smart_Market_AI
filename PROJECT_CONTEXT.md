...
- Next focus is to continue Phase 22 Research Score carefully: add opt-in ranking integration or move to Phase 21.6 / 21.7 external fetch UI while preserving unchanged default ranking order. Phase 21.5 Stock News RAG covers selected-symbol cockpit news evidence from local URL-backed news documents. Phase 21.6 / 21.7 have a first backend slice for explicit opt-in external research/news fetch adapters, Yahoo Finance adapter shape, local cache registration, chunk rebuild, manifest output, and network-free fake-adapter tests. Broader Investment News dashboard, Assistant, distribution readiness, and Execution/Broker integration remain planned or future scope unless explicitly assigned.

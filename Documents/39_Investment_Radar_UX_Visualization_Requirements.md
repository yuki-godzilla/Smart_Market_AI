...
- ヘッドラインは両タブの先頭で、重複除外済みの最大12件プールから常時3件を横並び表示する。4件目以降は8秒ごとに次の3件へ切り替え、ホバー中とreduced-motion環境では自動切替を止める。各カードは原記事URLがある場合にカード全体を別タブで開け、`HEADLINE FLOW`、プール件数、正確な最新公開時刻を併記する。見た目のライブ感は補助表現であり、リアルタイム配信やニュースの自動更新を意味しない。価格期間と分類軸は市場レーダー内だけで管理する。
...
- セクター／業種別で複数の1〜2銘柄分類が同時に出た場合は、`少数セクター`／`少数業種` の一枚へまとめる。ただし元の分類名をカード内に併記し、実測タイル・騰落率・Cockpit導線を失わない。少数分類が一つだけ残る場合は、分類名を保った低いコンパクトカードにして全面の大型マップにしない。注目ニュース別は根拠ニュースカードとの一対一の対応を保つため統合しない。
...
- 幅1280px以上のPC画面では分類グループを3列、タブレットでは1列に配置する。PCとタブレットは分類ごとに同じ実測タイル数（最大12件）を表示する。各グループ内はsquarified treemapを使い、横長の分類領域では短辺から優先して分割する。値動き上位のタイルを横並びに保ち、全幅の細い横帯が縦に連続して読みにくくならないようにする。iPhoneも同じ表示対象を縦カードへ切り替えて情報を省略せず、ページ全体の横スクロールを生じさせない。

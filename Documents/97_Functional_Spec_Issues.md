...
| FS-006a | Research Summary / ResearchBrief | Research Summary が生データ羅列で読めない | `ResearchBriefBuilder` が provider profile、news、TDnet trace、evidence を `ResearchBrief` / `ResearchMetric` に変換し、Cockpit / Ranking Research Summary 冒頭へ AI整理メモ、定量評価、企業概要、良材料候補、注意材料候補、不足根拠、次の確認資料、出典カードを表示する | 外部LLMは使わず local rule-based 生成を維持し、source confidence は情報源の信頼度として説明する。Research Score は調査メモの後ろへ寄せ、ranking order は変えない | ユーザーが何を確認すべきか分からない、provider raw fields を投資根拠と誤読する | UX readability | High | In review | AI整理メモの目立つカード、定量指標カード、不足指標パネル、出典信頼度バッジは実装済み。UI/browser確認で表示順と折り返しを確認 |

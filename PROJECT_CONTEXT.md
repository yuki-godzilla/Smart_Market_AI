...
- Continue Phase 18 by refining ETF metadata quality and source coverage after the JPX listed-stock Yahoo metadata refresh, JPX ETF/ETN official list import, and IMAJ NISA listed-fund metadata import. The current import command has JPX/SBI/NISA profiles, JPX listed-stock, JPX ETF/ETN, SBI US stock/ETF, and NISA eligibility raw-file builder support, NISA seed metadata applied to 31 existing rows, JPX NISA growth ETF/ETN metadata applied to 27 rows, IMAJ NISA growth metadata applied to 232 existing ETF rows, and 4,272 candidate-master rows while default ranking only includes stock / ETF rows.

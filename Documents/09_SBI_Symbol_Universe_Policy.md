...
- 投資信託や NISA 対応情報は商品ごとに更新粒度が異なる。
- ランキング機能の本質は、取得そのものではなく比較・分析・判断材料の整理である。

基本経路:

1. SBI公式サイト、JPX、投信協会、NISA対象商品リストなどで取扱銘柄・制度情報を確認する。
...
- 初期は主要ファンドのみを手動 CSV / source CSV で管理する。
- SBI の投信検索画面を直接スクレイピングしない。
- 信託報酬、純資産総額、投資対象地域、投資対象資産、インデックス / アクティブ、NISA成長投資枠、NISAつみたて投資枠、積立対応、分配方針を優先 metadata とする。
...
- 投信 / ETF / REIT の専用 metadata が増え、CSV列だけでは扱いづらくなった。

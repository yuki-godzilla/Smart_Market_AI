...
- 2026-05-27: Integrated Cockpit external Research fetch into the standard `AI調査を更新` flow: removed the separate external-permission panel from the UI path, fetches Yahoo Finance external profile/news with the backend safety gate when AI Research runs, stores source trace rows in session state, continues with saved/local fallback if external fetch fails, and displays external source trace rows inside the Research Summary panel / Decision Report without retaining fetched source text.

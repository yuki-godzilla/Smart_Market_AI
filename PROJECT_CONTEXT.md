...
Phase 14 has started with a Forecast Summary that shows median forecast, forecast range, and model agreement in the UI.
Phase 14 ? Forecast Summary ?????????median forecast?????model agreement ? UI ??????????????

## Next Good Targets / ??????

- Improve forecast chart readability with clear actual-vs-forecast styling, labels, and beginner-friendly metrics.
  ?????????????????????????????? forecast chart ??????????
- Continue Phase 14 by adding a lightweight model registry and connecting forecast summary to later scoring.
  Phase 14 ? model registry ? forecast summary ? scoring ?????????

...
- Domain services:
  - `backend/marketdata`
  - `backend/marketdata` 内の symbol universe / metadata schema / source import
  - `backend/forecast`
  - `backend/screening`
  - `backend/scoring`
  - `backend/risk`
...
- Symbol Universe / Metadata Layer
  - `symbol_universe.csv`、metadata schema、source import、provider-neutral refresh を管理する。
  - 初期 ranking universe は、SBI証券で取り扱いがあり、現物・NISA・長期投資で検討しやすい商品を前提にする。
  - FX、CFD、先物・オプション、暗号資産、債券、外貨建MMF、貴金属、レバレッジ、インバースは default policy で除外できるようにする。
- Forecast Service
...
- Research RAG Layer
  - 通常 checks は local fixture / fake adapter で deterministic に検証し、通常ユーザー導線では外部 source adapter から最新IR・開示・ニュース・provider 情報を一時取得/参照する。
  - 根拠 chunk、資料メタデータ、Research Score を Decision Report / Assistant / Investment Score に渡す。
  - 現在方針: 通常 checks は local fixture / fake adapter で network 非依存に保ち、ユーザー向けの `AI調査を更新` では TDnet、EDINET、企業IR、provider profile、news などの外部最新 source を優先する。local 登録資料は fixture、archive、private note、fallback として扱い、主たる live source にはしない。
  - Research Summary は、UI表示前に local rule-based `ResearchBrief` で読める調査メモへ整形する。次の planned slice では `ResearchFactSummary` を前段に置き、事業概要、IR / 公式資料、主要指標、直近イベント、良材料候補、注意材料候補、未確認項目を source-backed fact として整理する。外部LLM要約は future / optional とする。
...
1. Symbol Universe / Metadata Layer が ranking 候補を local master と policy で絞る。
2. MarketData provider が `Bar` / `Quote` / `FxRate` へ正規化した市場データを返す。
3. Feature Store Lite が銘柄ごとの特徴量 snapshot を作る。
4. Screening Service が特徴量から候補銘柄を ranking する。
5. Forecast Service が複数モデルの予測を実行する。
6. Research RAG Layer がIR資料・決算資料・ニュースから evidence と Research Score を生成する。
7. Investment Score Service が screening、forecast、risk、data quality、optional research を統合する。
8. API / UI / report がスコア、予測、根拠、不確実性を表示する。
...
**目的**: NISA対応を含む個人投資家向けの株式・ETF候補整理、市場予測、リスク分析を、ローカル中心で高速に提供。
...
- **投信データ**の標準化ソース（信託報酬、基準価額、リスク指標）。これは Future Phase。

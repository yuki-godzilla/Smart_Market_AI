...
LLMFactorExecutionMode = Literal["auto", "light", "quality", "off"]
LLMFactorEnvironmentProfile = Literal["notebook", "desktop", "server", "offline"]
LLMFactorProfileName = Literal[
    "notebook_dev",
    "notebook_standard",
    "desktop_fast",
    "desktop_analysis",
...
class LLMFactorEvidenceContext(StrictBaseModel):
...
class LLMFactorGenerationContext(StrictBaseModel):
...
class LLMFactorGenerationConstraints(StrictBaseModel):
...
class LLMFactorGenerationRequest(StrictBaseModel):
...
class LLMFactorGatewayFactor(StrictBaseModel):
    title: str = Field(min_length=1)
...
class LLMFactorGatewayRisk(StrictBaseModel):
    title: str = Field(min_length=1)
    summary: str = Field(min_length=1)
...
class LLMFactorGatewayOpportunity(StrictBaseModel):
    title: str = Field(min_length=1)
    summary: str = Field(min_length=1)
...
class LLMFactorGatewayEvidence(StrictBaseModel):
    evidence_id: str = Field(min_length=1)
    title: str = Field(min_length=1)
    source_type: str = Field(min_length=1)
    source_url: str | None = Field(default=None, min_length=1)
...
class LLMFactorGenerationResponse(StrictBaseModel):
    schema_version: str = Field(
        default=LLM_FACTOR_GATEWAY_RESPONSE_SCHEMA_VERSION,
        min_length=1,
    )
    ...
    profile: LLMFactorProfileName = "fallback"

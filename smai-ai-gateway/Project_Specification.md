...
| SMAI Copilot workspace | 親側実装済み | 画面横断の相談 UI | SMAI 側サイドメニューに chat-style workspace を追加済み。context preset、限定自由入力、user / assistant bubbles、session-local 履歴を持ち、Gateway は `/api/v1/context-answer` の汎用境界として使う | 親側 roadmap |
...
- `SMAI Copilot` チャット画面、限定自由入力、user / assistant bubbles、session-local 会話履歴の first MVP は親側で実装済み。実 Gateway / Ollama live smoke と、長い会話履歴・複数文脈参照の本格拡張は未実行 / 後続範囲。
...
| SMAI Copilot workspace | 親側実装済み | 親SMAIのサイドメニューに、context preset、限定自由入力、user / assistant bubbles、session-local 履歴を持つ chat-style Copilot workspace first MVP を追加済み。Gateway 側は汎用 HTTP API 境界のまま。 |

...
`投資ヒントとなるニュース` は、URL付きの一般ニュースだけを切り出すカードです。Google News RSS や Yahoo Finance news 由来の一般ニュースを対象にし、TDnet、企業IR、EDINET、Yahoo Finance profile などの source trace と混ぜません。IR/開示の根拠カードとは別系統の軽いヘッドラインUIにし、タイトル、公開日、鮮度、出典、材料分類、確認観点、短い要約を優先します。raw URLは本文表示せず、カード全体をクリックして外部URLを開く導線にします。
...
Cockpit Research Summary の `投資ヒントとなるニュース` はURL付き一般ニュース専用です。カードには `タイトル`、`公開日`、`鮮度`、`出典`、短い `要約` を置き、カード全体をクリックしてニュース本文を開けるようにします。ニュースが売買推奨やランキング順位の材料そのものに見えないよう、長い解説やraw URLは通常カードから外します。TDnetや公式IRは `ニュース・開示の出典` と `外部参照ソース` 側で扱います。

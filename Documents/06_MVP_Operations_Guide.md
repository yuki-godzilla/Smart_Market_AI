...
- Low-cost Assistant Streamlit / Agentic first slices
  ...
  - `投資レーダーのニュースを更新して`などの明示依頼では、Assistantは実行前確認を表示する。`ニュースを更新`を押した後だけ既存News refresh/cache経路をforce実行し、成功、前回保存データ利用、失敗を区別する。結果表示はニュース件数、カテゴリ数、JST更新時刻に限定し、raw本文やProvider応答はAssistant履歴へ保存しない。キャンセル時は外部通信しない。
...
- Ranking LLM Interpretation
  - Phase 28-B adds `Ranking AI解釈（参考）` after the top candidate cards. The feature is disabled by default. Enable it with a local config based on `config/ranking_interpretation_example.yaml`; `llm_interpretation.ranking.enabled=true` does not generate automatically, and the user must also press `AI解釈を生成`.
  - The parent sends at most eight compact sections: Ranking scope, primary metrics, current top-candidate sector groups, and up to five displayed candidates. Provider raw fields, external bodies, user notes, tags, and debug data are not included. `user_id + context_hash` separates Streamlit results across user/context changes.
  - Gateway uses `/api/v1/context-answer`, `task_type=ranking_interpretation`, and `response_schema=ranking_interpretation.v1`. Every output point must cite supplied evidence IDs. Wrong context/candidate/order, duplicate candidate, unknown evidence/number/date, advice wording, malformed JSON/schema, timeout, and provider failure reject the whole live payload. Only validated `live` results are atomically cached in `data/cache/ranking_interpretation_results.json` for the configured TTL (default six hours).
  - Disabled and failed states show a deterministic reading guide. Neither live nor fallback output changes Ranking rank/score, Forecast, AI総合, Investment Score, Research Score, Decision Report, portfolio, or execution behavior. Normal tests use mock clients and remain network-free.
- Investment Radar Overview Interpretation
  - Phase 28-C2では、市場ヒートマップの後に`投資レーダー AI読み解き（参考）`を表示する。既存の候補単位`radar_interpretation.v1`とは別契約である。`config/radar_overview_interpretation_example.yaml`を参考にローカル設定で有効化する。`llm_interpretation.radar_overview.enabled=true`でも自動生成せず、`AIで今日のレーダーを整理`の明示操作が必要である。
...
- `上向き兆候` is an independent Ranking exploration policy. It combines chart shape 30%, forecast upside room 25%, downside safety 20%, pullback/stability 10%, company/dividend quality 10%, and upward material 5%. Data quality remains a reliability note and a block-level unavailable check, not an attractiveness boost. After staged danger deductions, the score expands the crowded midrange around 50 on a smooth 0--100 curve so users can distinguish candidates without flattening weak or dangerous ones. It never overwrites `total_score`. Ranking, Cockpit, Myウォッチリスト snapshots, Ranking History, Assistant context, and Decision Report retain backward-compatible internal `reversal_*` fields while displaying `上向き兆候`. Treat it as a further-review priority, not buy advice.
  - When selected, the Ranking condition card shows the difference from `上昇気配` and the exact 30/25/20/10/10/5 formula. `上向き兆候スコアの詳しい計算方法を見る` shows component inputs, chart-shape patterns, pullback bands, staged danger deductions, and the midrange expansion. The map only uses a two-dimensional pair when both axes vary; otherwise it tries the next candidate axis and then shows the `上昇気配 × 下降警戒` alternative map.
...
- Cockpit `AI解釈メモ` は Phase 28-A、Ranking `Ranking AI解釈（参考）` は Phase 28-B、Investment Radar OverviewはPhase 28-C MVPとして実装済み。いずれも既定disabledかつ明示実行のみで、順位・score・Forecastを変更しない。News / Research Summary / Decision Reportへの展開、Assistantからの画面説明連携、Decision Reportへの自動挿入は後続範囲

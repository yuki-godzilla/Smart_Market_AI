...
10. SBI / NISA / future 投信 metadata source import. 部分完了。`--source-profile`、JPX stock/ETF profile、SBI US stock/ETF seed、NISA eligibility update profile/seed、mutual fund seed、投信 metadata columns の import path は追加済み。JPX 東証上場銘柄一覧 raw file から国内株 source CSV を作る `build_symbol_universe_source.py --source-kind jpx_listed_stock` も追加済み。MVP ranking は stock / ETF のみ対象にし、mutual fund seed は future extension として保持する。次は SBI US / ETF / NISA の公式 source 変換を拡張する。
...
- `tools/build_symbol_universe_source.py` converts official raw files into local source CSVs. The first builder supports JPX listed-stock raw Excel/CSV and outputs stock-only source rows for `--source-profile jpx_listed_stock`; ETF / ETN / REIT rows are skipped for a separate source path.

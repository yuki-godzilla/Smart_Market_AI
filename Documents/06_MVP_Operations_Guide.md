...
- Streamlit startup runs a bounded local-first symbol DB refresh from `symbol_universe.csv`. It writes latest-only normalized records to `data/cache/symbols_cache.json`, keeps `symbol_refresh_queue.json` empty after successful batches, and updates `symbol_refresh_status.json`. The default startup batch is small (`MAX_SYMBOL_REFRESH_PER_RUN = 20`), uses no external network, skips fresh records, and advances remaining missing/stale symbols on later app starts.

...
`投資ヒントとなるニュース` は、URL付きの一般ニュースだけを切り出すカードです。Google News RSS や Yahoo Finance news 由来の一般ニュースを対象にし、TDnet、企業IR、EDINET、Yahoo Finance profile などの source trace と混ぜず、材料分類、確認観点、鮮度、要約、なぜ見るか、追加確認、URLを短く示します。
...
- `投資ヒントとなるニュース`
- `ニュースだけを切り出し、業績・成長・株主還元・リスクなどの確認観点で整理します。売買推奨ではなく、次に確認する材料として扱ってください。`
- `なぜ見るか`
- `追加確認`
...
Cockpit Research Summary の `投資ヒントとなるニュース` はURL付き一般ニュース専用です。カードには `要約`、`なぜ見るか`、`追加確認` を置き、ニュースが売買推奨やランキング順位の材料そのものに見えないようにします。TDnetや公式IRは `ニュース・開示の出典` と `外部参照ソース` 側で扱います。

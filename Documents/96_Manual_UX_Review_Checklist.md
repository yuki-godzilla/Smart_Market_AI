...
| UX-N04 | 通知センター / 右上ユーザーエリア | 全画面から通知と表示ユーザーを確認する | 未読/重要件数、filter、通知カード、既読/archive、ユーザー切替、マスコット、登録端末を操作する | 通知センターがサイドメニューを増やさず右上から開き、Trusted Deviceは認証ではないと分かる | スマホで横overflow、色だけの重要度、端末記憶を認証と誤認、CTAが処理を自動開始 | Security / Responsive UX | High | In review | network-free UI契約test実装済み。4 viewportとlocalStorage再訪の実画面確認はbrowser利用時に行う |

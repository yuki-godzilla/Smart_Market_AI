...
- `売買推奨ではありません`、情報源の信頼度、不足指標の有無はメモの近くにバッジで表示する。
- Research Evidence の上部操作カードは `AI調査ステータス` のような取得管理表示ではなく、`AI調査でわかったこと` として事業概要、確認した資料、主な材料、次の確認ポイントを自然文で表示する。
- `資料n件`、`根拠n件`、`出典カードn件` は補助情報に留め、主表示の本文には事業概要、確認済みIR / 公式資料、主要指標、直近イベントなどの fact を置く。
- provider-only の情報は `外部プロバイダー情報では` と明示し、公式IR、TDnet、EDINET、企業IRと同じ信頼度に見せない。
- `読み方サマリー` は `確認できたこと`、`注意して見ること`、`まだ足りないこと`、`次にやること` の4観点に絞る。詳細な根拠リストではなく、最初に読む順番を示す。
- `確認ポイント` は事業概要、良材料候補、注意材料候補を最大2件ずつに絞り、長い provider text や raw fields を初期表示に出さない。
- `良材料候補` / `注意材料候補` は「どの材料を何件確認したか」「主な出典は何か」を示す。各候補には `公式資料` / `外部provider` / `ニュース` などの出典ランクと `情報源信頼度: 高/中/低/未確認` を近くに表示する。英語抜粋や provider raw fields をそのまま貼らない。
...
- `出典カード`、`Research Score（参考）`、`外部参照ソース` は折りたたみ表示にして、score や forecast と区別して読む文脈にする。
...
| `Research Score` / `根拠スコア` | 根拠資料の充実度・鮮度・信頼度を整理する参考スコア | 良い銘柄・買い候補の点数ではなく、資料に基づく確認材料のそろい具合として説明する。既定では Investment Score や ranking order を変えない |

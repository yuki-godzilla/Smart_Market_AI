...
Phase U1以降、`default / SMAIデフォルト` は通知対象外とする。UI非表示だけでなく、
設定保存、履歴作成、Producer、scheduler、テスト通知、gateway送信でも拒否する。
N6の実お気に入りProducerは共有JSONを参照せず、対象user_idのFavorite Storeを入力にする。
U1-Verifyではdefaultの履歴、NotificationService、テスト送信、catalog Producer、
gateway送信の拒否と、system userのscheduler対象除外を確認した。N6開始前提は満たしているが、
実favorite/news/market Producer自体は本フェーズでは追加しない。
...
実装状態: Phase N4実装済み。schema v5へ`app_notifications`、`delivery_results`、`users`、通知カテゴリ設定を追加し、N3設定を保持したままmigrationする。`NotificationService`は履歴保存成功後だけ外部clientを呼び、テスト通知だけをProducerとして接続する。右上固定ユーザーエリアはユーザー設定、通知設定、ユーザー切替の入口に限定し、通知先とアイコンはユーザー設定へまとめる。サイドメニューには通知画面を追加しない。

### Trusted Device

2026-06-30のUI回帰修正で自動ユーザー選択と登録端末管理は廃止した。既存SQLite tableはmigration互換のため残すが、通常UIとユーザー解決では参照しない。新しいbrowser sessionはプロフィール選択画面を経由する。
...
- N6第4 sliceでは、`favorite_move_alert`だけを対象に、custom userの保存済みWatchlist snapshotから`status=ok`、有限な1日騰落率、timezone付き`last_price_at`、実行時点から90分以内の測定値だけを読む。provider refresh、cache/profile書込み、Ranking / Forecast / Score再計算は行わない。不正・失敗・欠損・期限切れのsnapshotはsampleへfallbackせず`no_fresh_marketdata_measurement`などのsanitized reasonでskipする。同一のsymbol / 丸めた騰落率 / 取得時刻の集合はhashでdedupeし、測定内容が変わるまで15分schedulerで再通知しない。
- N6第5 sliceでは、`favorite_daily_report`だけを対象に、同じcustom userのsnapshotから`status=ok`、timezone付き`last_price_at`、有限な価格または1日騰落率、実行時点から36時間以内の計測を数える。0件ならsampleへfallbackせず`no_fresh_marketdata_measurement`でskipし、1件以上なら`有効計測数 / お気に入り総数`を表示する。市場別の取引時間・休場日・現在価格は推測せず、provider refresh、cache/profile書込み、Ranking / Forecast / Score再計算は行わない。日次slotの既存dedupeを維持する。
- N6第6A/B sliceでは、`favorite_move_alert`の市場時間判定を端末ローカル時刻の一律条件から、保存済み`market` / `asset_type`だけで解決する銘柄単位のregular-session policyと、versioned local calendarへ移す。日本株は`Asia/Tokyo`の09:00–11:30 / 12:30–15:30、米国株は`America/New_York`の09:30–16:00を用い、DSTはIANA timezoneで扱う。`config/notification_market_calendar.v1.json`は2026年のreview済みJPX / NYSE休場日、米国株の短縮取引（11月27日・12月24日13:00 ET）とcoverageを持つ。市場不明、非対応asset、週末、休場日、時間外、calendar未カバー、calendar読込不能はsampleへfallbackせずsafe reasonでskipする。schedulerは同一のtimezone付き`evaluated_at`をadapterへ渡し、`favorite_move`だけはserver timezoneの週末gateを使わず市場カレンダーを正とする。tickerから市場を推測せず、Provider取得、cache/profile書込み、Ranking / Forecast / Score変更は行わない。翌年のcoverageはreview済みseedとして別途更新し、未カバー日はfail-closedとする。
- schedulerの`--dry-run`はdue jobとsafeなready / skipped reasonだけをJSONで表示し、通知履歴、run claim/log、ntfy配送を作らない。debug manual generatorはcatalog sampleを維持する。

...
モデル変更UIはchat input横の単一 `AIモデル` selectboxです。選択肢には取得済みmodel名と特色・負荷感をまとめて表示し、用途profileの別radioや内部名（`notebook_dev` など）は表示しません。初回は取得済みmodelの性能順topを選び、ユーザーが変更した後はそのmodelをsession内で保持します。

...
- Investment Radarの`radar_interpretation.v1`とRankingの`ranking_interpretation.v1`は、candidate/context IDと項目別`cited_evidence_ids`を持つstrict JSONをGatewayから返す。Rankingでは固定済みcandidate順も照合する。Gatewayは許可済みcontext IDだけを参照し、親SMAIがsymbol・数値・日付・助言表現のgrounding validationとdeterministic fallbackを担う

...
10. SBI / NISA / future 投信 metadata source import. 部分完了。`--source-profile`、JPX stock/ETF profile、SBI US stock/ETF seed、NISA eligibility update profile/seed、ranking metadata update profile、mutual fund seed、投信 metadata columns の import path は追加済み。JPX 東証上場銘柄一覧 raw file から国内株 source CSV を作る `build_symbol_universe_source.py --source-kind jpx_listed_stock`、JPX 国内 ETF / ETN source CSV を作る `--source-kind jpx_etf`、SBI米国株 / 米国ETF・海外ETF raw file から source CSV を作る `--source-kind sbi_us_stock` / `sbi_us_etf`、NISA raw file から制度 metadata source CSV を作る `--source-kind nisa_eligibility` も追加済み。JPX NISA 成長投資枠 ETF/ETN Excel は 27件を `symbol_universe.csv` に反映済み。MVP ranking は stock / ETF のみ対象にし、mutual fund seed は future extension として保持する。次は公式 ETF 全銘柄一覧の Excel/CSV 化、SBI米国株/海外ETF source の拡張、NISA一覧の継続更新、確認済み PER/PBR/ROE/配当利回り source の補完を進める。
...
- `tools/build_symbol_universe_source.py` converts official raw files into local source CSVs. It supports JPX listed-stock raw Excel (`.xls` / `.xlsx`) / CSV for `--source-profile jpx_listed_stock`, JPX ETF / ETN raw Excel (`.xls` / `.xlsx`) / CSV for `--source-profile jpx_etf`, SBI US stock / US ETF raw CSV/Excel for `--source-profile sbi_us_stock` / `sbi_us_etf`, and NISA eligibility raw CSV/Excel for `--source-profile nisa_eligibility`. JPX listed-stock builder maps `規模区分` into `market_cap_tier` for the ranking UI. ETF builders preserve commodity theme and leveraged / inverse flags so ranking policy can exclude them. NISA builder supports JPX growth-NISA Excel headers with furigana, treats that list as growth-eligible, and preserves ambiguous rows as `unknown` instead of over-inferring a category. PDF raw files are not part of the routine import path.

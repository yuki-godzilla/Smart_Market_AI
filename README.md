...
- Phase 23: Optional Adapter / 高度分析を先に進める。Advanced Forecast Slice 1 として、`advanced_linear` forecast adapter の backend first slice を実装済み
...
次の重点は Phase 23 Advanced Forecast Slice 1 の接続拡張です。backend には `advanced_linear` adapter が入り、Ridge 系の軽量・deterministic な予測モデルで 5 / 20 trading day forward return、walk-forward validation、validation metrics、confidence、feature contribution summary を扱えます。次は forecast service / API / Streamlit の最小接続を進めます。Phase 18 の source 更新や残 metadata gap 補完は運用タスクとして継続し、Assistant / optional LLM/provider、Execution / Broker は段階的に扱います。通常 checks は引き続き fake adapter / fixture で network 非依存を維持します。

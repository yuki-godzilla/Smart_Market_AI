...
10. SBI / NISA / future 投信 metadata source import. 部分完了。`--source-profile`、JPX stock/ETF profile、SBI US stock/ETF seed、NISA eligibility update profile/seed、mutual fund seed、投信 metadata columns の import path は追加済み。JPX 東証上場銘柄一覧 raw file から国内株 source CSV を作る `build_symbol_universe_source.py --source-kind jpx_listed_stock` と、SBI米国株 / 米国ETF・海外ETF raw file から source CSV を作る `--source-kind sbi_us_stock` / `sbi_us_etf` も追加済み。MVP ranking は stock / ETF のみ対象にし、mutual fund seed は future extension として保持する。次は NISA公式一覧の source 変換と、公式ファイル取得を手動運用から半自動運用へ寄せる導線を検討する。
...
- `tools/build_symbol_universe_source.py` converts official raw files into local source CSVs. It supports JPX listed-stock raw Excel/CSV for `--source-profile jpx_listed_stock` and SBI US stock / US ETF raw CSV/Excel for `--source-profile sbi_us_stock` / `sbi_us_etf`. JPX ETF / ETN / REIT rows are skipped by the JPX stock builder for a separate source path. The SBI ETF builder preserves leveraged / inverse flags so ranking policy can exclude them.

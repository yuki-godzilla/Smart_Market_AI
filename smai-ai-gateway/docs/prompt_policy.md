...
Gateway は provider 呼び出しと prompt 実行の境界を担い、`LLMFactorResult` などの SMAI domain schema、source hash、file-backed cache、deterministic backtest evaluator は SMAI 本体側の初期 slice で扱います。Ranking 参考表示、broader historical backtest、cache policy expansion、UI 統合拡張も SMAI 本体側で進めます。

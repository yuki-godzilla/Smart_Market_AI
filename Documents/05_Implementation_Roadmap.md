...
| Phase N1〜N4 / Notification Platform | 🟦 **N1〜N4実装済み / N5 Producer未着手** | schema v5通知履歴、保存優先service、固定右上ユーザー入口、ユーザー別通知カテゴリ、プロフィール選択、明示テスト通知まで実装。Trusted Device自動選択は廃止。自動Producerは未接続。 |
...
- 🟨 **N1〜N3-B実装済み / N4未着手**: Phase N1〜N4 通知基盤。独立gateway、親client/adapter、versioned SQLite設定、既存設定画面のntfy設定、明示テスト通知まで実装済み。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。既存イベント接続、自動通知、アプリ内履歴、通知センターはまだ行わない。

...
- R10.6: InvestmentQuestionSummary / 投資判断で知りたいこと。初期 slice は実装済み。`CompanyResearchReport`、`ResearchBrief`、`InvestmentInsight`、`ExternalResearchFetchResult`、`StockNewsReport` から、事業モデル、売上・利益トレンド、収益性、今期見通し、成長ドライバー、リスク、株主還元、バリュエーション、直近ニュース影響、最重要論点の10質問を必ず生成する。情報がない場合は `判断できません` / `未取得` と明示し、根拠レベルを 高 / 中 / 低 / 不足 で表示する。売買推奨や割安・割高の断定はしない。
...
- Cockpit Research Summary の主表示順は、SMAI 投資判断サマリー -> 投資判断で知りたいこと（初期5項目 + その他5項目の折りたたみ）-> 3カラム判断材料（良い材料 / 注意材料 / まだ判断に足りない情報）-> folded 根拠確認（AI整理メモ / 読み方サマリー / 会社概要 / 確認できた事実 / 公式資料で未確認）とする。出典カード、Research Score、件数、confidence、詳細データは必要時に開く補助情報へ下げる。

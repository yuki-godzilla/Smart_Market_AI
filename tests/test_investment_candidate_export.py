...
def _universe() -> list[dict[str, str]]:
    rows = []
    for market, symbol, tier in (("jp", "1111.T", "large"), ("jp", "2222.T", "small"), ("us", "USONE", "large"), ("us", "USTWO", "mid")):
        rows.append({"symbol": symbol, "name": f"Company {symbol}", "market": market, "asset_type": "stock", "nisa_category": "growth", "market_cap_tier": tier, "risk_band": "MEDIUM", "dividend_yield_pct": "3.2", "per": "15", "pbr": "1.2", "roe_pct": "12", "is_active": "true"})
    return rows
...
def _runner(symbols, _start, _end, _provider, _progress):
    ...
    return ([{"symbol": symbol, "total_score": "72", "screening_score": "68", "risk_score": "30", "data_quality": "OK", "upside_signal": "61", "downside_warning": "21", "volatility": "14", "warnings": ""} for symbol in symbols], [])


def test_candidate_definitions_cover_the_requested_14_region_separated_exports():
    assert len(RANKING_DEFINITIONS) == 14
    assert [item.ranking_id for item in RANKING_DEFINITIONS] == [f"{index:02}" for index in range(1, 15)]
...
def test_export_writes_bom_csv_combined_overlap_zip_and_retains_partial_failures(tmp_path):
    summary = export_investment_candidates(output_root=tmp_path, universe_rows=_universe(), runner=_runner, top_n=1, fetch_limit=300, now=datetime(2026, 7, 17, 2, 0, tzinfo=UTC))

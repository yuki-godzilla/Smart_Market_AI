...
| UX-008 | Ranking | AI Research を使う | `AI Research` タブで `AIで資料を確認` を押す | 登録資料から確認材料を整理する機能だと分かる | AIが投資判断を代行するように見える | Wording Risk | High | Not tested | Research Evidence 注意書き |
...
| UX-014 | Research Summary / Evidence | 資料がある銘柄で Research Score を確認する | 登録資料ありの銘柄で Research Summary / Research Score / 根拠カードを表示する | 資料名、公開日、根拠数、鮮度、Research Score の内訳と confidence が確認でき、売買推奨ではなく根拠資料の充実度だと分かる | 根拠の鮮度や信頼度が分かりにくい、または高スコアが「買い」に見える | Data Confidence | High | In review | 公式資料、provider snapshot、外部一時取得の区別も確認 |
| UX-015 | Research Summary / Evidence | 資料がない銘柄で Research Score を確認する | 資料未登録の銘柄で Research Summary を実行し、Research Score 表示を確認する | 根拠不足として控えめに扱われ、低信頼または未取得の参考情報だと分かる。既定の Investment Score / ranking order は変わらない | 情報なしが低評価や売買判断に見える | Spec Ambiguity | Medium | In review | `scoring.weights.research=0.0` の既定挙動も確認 |

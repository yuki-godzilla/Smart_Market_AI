...
| FS-006a | Research Summary / ResearchBrief | Research Summary が生データ羅列で読めない | provider profile、news、TDnet trace、evidence を UI に近い形で表示している | `ResearchBrief` / `ResearchMetric` へ変換し、AI整理メモ、定量評価、企業概要、良材料候補、注意材料候補、不足根拠、次の確認資料、出典カード、詳細データの順に表示する。外部LLMは使わず local rule-based で生成する | ユーザーが何を確認すべきか分からない、provider raw fields を投資根拠と誤読する | UX readability | High | Planned | provider raw fields は通常表示から外し、詳細データに限定。Research Score は先頭に置かない |

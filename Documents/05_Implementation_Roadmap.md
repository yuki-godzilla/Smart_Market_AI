...
| Phase U1 / Local User Profiles | 🟦 **実装済み / 実画面継続確認** | ローカルユーザー追加、ユーザー別favorites/snapshot、defaultのsession-onlyお気に入り、default通知のUI/サービス無効化、旧共有JSONの安全な一度限り移行を実装。N6実データProducerはユーザー境界を必須とする。 |

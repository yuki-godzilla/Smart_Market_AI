...
- SMAI 本体側の `SMAI LLM Factor` 初期 schema / fake service / file-backed cache / Cockpit 参考表示は親プロジェクト側で実装済み。Gateway 側にはまだ構造化抽出 endpoint を追加しない。
...
- `LLMFactorResult`、factor schema、source hash、file-backed cache は SMAI 本体側の初期 slice で実装済み。backtest、Ranking 参考表示、cache policy expansion、UI 統合拡張も SMAI 本体側で扱い、Gateway から SMAI module は import しない

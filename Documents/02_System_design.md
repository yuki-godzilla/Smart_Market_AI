...
- Research RAG Layer
  ...
  - Research Summary は、UI表示前に local rule-based `ResearchFactSummary` / `ResearchBrief` で読める調査メモへ整形する。`ResearchFactSummary` は、事業概要、IR / 公式資料、主要指標、直近イベント、良材料候補、注意材料候補、未確認項目を source-backed fact として整理する。外部LLM要約は future / optional とする。

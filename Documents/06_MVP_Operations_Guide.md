...
追加の30-H recovery sliceでは、warming / retrying / model loading中だけmain領域をモーダルで覆い、sidebarは操作可能なまま維持します。Gateway未接続、provider未接続、model missing、timeoutを区別し、既定で最大3回を軽いbackoff付きで再確認した後はfallbackへ移行します。fallback画面からは `LLM接続を再確認`、`モデル一覧を再取得`、`fallbackで続ける` を利用でき、復旧時は履歴とworkflow sessionを保ったまま通常チャットへ戻ります。

モデル選択はGateway `GET /models` の実在モデル一覧を使います。優先順位は画面での明示選択、前回選択、設定モデル、利用可能な中で最も高性能なモデル、fallback既定値です。モデル一覧取得失敗時も固定fallbackで画面を利用でき、選択中モデルが消えた場合は警告します。チャットはメッセージ件数が増えた時だけ末尾へ自動スクロールし、通常rerunではスクロールを強制しません。

モデル変更UIは単一の `利用可能モデル` radio listです。モデル名、特色、用途、負荷感、`自動選択` / `選択中` を同じ選択肢に表示し、用途profileの別radioや内部名（`notebook_dev` など）は表示しません。通常時は `AIモデル: <model> / <特色>` のコンパクト表示で、開いた時だけ一覧、最終取得時刻、再取得・再接続操作を表示します。自動選択順位は明示選択、前回選択、config、利用可能な中で最も高性能なモデル、fallback defaultです。

...
The next Research Summary maturity direction is local-first readability, not external LLM integration. Add a display-only `ResearchBrief` / `ResearchMetric` layer that converts RAG evidence, provider profile, news, and TDnet traces into a readable AI整理メモ, quantitative summary, business overview, positive / caution candidates, missing evidence, next actions, and source cards. Provider raw fields should stay out of the normal view and remain only in detail rows.
...
- Phase 19 is complete; Phase 20 local Research RAG evidence work is implementation-complete; Phase 21 Advanced Research RAG / Stock News RAG MVP and the first Phase 22 Research Score / disabled-by-default Investment Score input / Cockpit Decision Report slices are implemented; later Phase 22+ ranking integration, assistant, optional adapters, and execution gate remain planned.

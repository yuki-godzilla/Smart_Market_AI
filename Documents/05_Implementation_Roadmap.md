...
| Phase N1〜N6 / Notification Platform | 🟦 **N1〜N5-C foundation実装済み / N6第6A slice完了** | schema v5通知履歴、型付きcatalog、設定/dedupe対応Producer、専用通知センター、手動生成、opt-in scheduler/job registry/run log/runnerまで実装。N6第1 sliceは、user-scoped favorites/watchlist snapshotsとfresh News cacheをread-only adapterでschedulerへ接続し、データ不足・stale・対象外はsampleへfallbackせずskip logへ記録する。第2 sliceは明示Cockpit AI調査完了時だけ、report fingerprintによるdedupe済み `smai_analysis_complete` を作成する。第3 sliceはcustom userの明示Assistant Report保存後だけ、sanitized artifact hashでdedupeした `smai_report_ready` を作成する。第4/5 sliceは有効・鮮度内のWatchlist市場計測をFavorite急変通知と日次coverageへ接続する。第6A sliceは急変通知を保存済み市場属性による日本・米国のregular sessionへ限定し、端末時刻・ticker推測には依存しない。休日・短縮取引カレンダーはN6第6Bとして未導入。default user、画面rerun、preview、downloadでは発火しない。 |
...
- 🟦 **N1〜N5-C foundation実装済み / N6第6A slice完了**: 独立gateway、親client/adapter、versioned SQLite設定/履歴、catalog、Producer、専用通知センター、manual generation、opt-in scheduler基盤を実装。N6第1 sliceは実favorite/news/sector cache adapterをschedulerへ接続済みで、第2 sliceは明示Cockpit AI調査完了event、第3 sliceはcustom userの明示Assistant Report保存eventを接続した。第4/5 sliceは鮮度内のWatchlist市場計測をFavorite急変通知と日次coverageへ接続する。第6A sliceは保存済み市場属性だけで日本・米国のregular sessionを判定し、time zone/DSTを適用して急変通知を時間外・週末・市場不明から抑止する。休日・短縮取引カレンダーはN6第6Bとして未導入である。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。

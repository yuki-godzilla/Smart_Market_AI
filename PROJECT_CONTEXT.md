...
Current Research Summary maturity work is local-first readability, not external LLM integration. A display-only `ResearchBrief` / `ResearchMetric` layer converts `CompanyResearchReport`, RAG evidence, provider profile snippets, news, and TDnet traces into a readable AI整理メモ, reading guide, quantitative summary, business overview, positive / caution materials, missing evidence, next actions, and source cards. Provider raw fields and English snippets are shaped into Japanese confirmation text for the primary view; the operation card and reading guide summarize what was confirmed, what sources were checked, what to watch, what is missing, and the next action before detailed cards. Material cards show source rank and source confidence, confirmation gaps use beginner-friendly wording, and raw/source details remain behind expanders when needed. If rule-based shaping becomes insufficient, a local lightweight LLM summarizer can be considered in a later phase while keeping normal checks deterministic.

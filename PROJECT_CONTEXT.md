...
- Reporting/export foundations exist for screening; forecast export remains a likely near-term target.
  screening ??? reporting/export ??????forecast export ???????????
...
Phase 13 is the active area: forecast baseline models, chart visibility, model comparison, and export/reporting polish.
...
- Improve forecast chart readability with clear actual-vs-forecast styling, labels, and beginner-friendly metrics.
  ?????????????????????????????? forecast chart ??????????
- Add forecast result export/reporting so UI-visible analysis can be saved and compared.
  UI ????? forecast ??????????? export/reporting ???????

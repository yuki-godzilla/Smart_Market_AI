...
- 投資レーダーの `直接言及`、`SMAI推測候補`、`マクロ確認用` は由来が異なるため同じラベルや色で混ぜない。`確認優先度` は次に根拠を確認する順序であり、魅力度・期待収益・おすすめ度・順位として説明しない。
...
投資レーダー内のヒートマップ見出しは `投資ヒートマップ` とします。見た目は株式ヒートマップ風に、投資カテゴリをセクター枠、関連銘柄をタイルとして表示してよい。タイルには銘柄名 / 企業名とシンボルを併記し、企業名を主表示、シンボルを小さな補助タグとして扱います。長い名称は省略しつつ hover title で確認できるようにします。タイルクリックは該当銘柄の `銘柄コックピット` に移動する導線とし、ニュース画面だけで判断を閉じない流れにします。ただし、値動きや取引量の色は市場の温度感を読むための確認材料であり、売買判断や銘柄推奨そのものに見えない説明を近くに置きます。市場指標がないカテゴリにニュース材料から補完値を出す場合は `ニュース代理` / `材料+1.2` のように、実測の値動きではなく確認用シグナルであることが分かる文言にします。

...
- `SMAI Copilot` の実 Gateway / Ollama を起動した手元 live smoke 実行と、会話履歴 / 参照文脈の本格拡張。専用チャット画面、限定自由入力、SMAI から `smai-ai-gateway` への HTTP client wiring、`SMAIアシスタント` 画面の既定 LLM Gateway 接続、Gateway側のLLM構造化JSON応答、親SMAI側の opt-in live smoke test path は実装済み

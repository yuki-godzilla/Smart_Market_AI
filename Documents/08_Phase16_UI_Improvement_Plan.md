...
- `upside_signal`: `Upside x Downside Watch Map`。`上昇気配` と `下降警戒` を主軸にし、差し引き後の方向感を色で確認する。
...
Symbol Cockpit は、Summary / KPI の直後に `02 Price & Forecast` を置き、価格・予測チャートを1銘柄深掘りの主役として扱う。ランキングで使う方向シグナルを `03 Direction Signal / 上昇気配・下降警戒` としてチャート直後に置き、`04 Score Breakdown` は値動きと方向感を見たあとに「なぜこの評価になっているか」を確認する位置づけにする。
...
- `03 Direction Signal` で `方向感`、`上昇気配`、`下降警戒`、モデル方向一致、予測のばらつきを確認する。

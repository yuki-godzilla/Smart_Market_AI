...
Phase 14 has Forecast Summary, ensemble forecast, and Model Registry Lite for deterministic baseline model selection and labels.
Phase 14 ? Forecast Summary?ensemble forecast?Model Registry Lite ??????????????????????????????

## Next Good Targets / ??????

- Improve forecast chart readability with clear actual-vs-forecast styling, labels, and beginner-friendly metrics.
  ?????????????????????????????? forecast chart ??????????
- Continue Phase 14 by connecting forecast summary to later scoring.
  Phase 14 ? forecast summary ? scoring ?????????

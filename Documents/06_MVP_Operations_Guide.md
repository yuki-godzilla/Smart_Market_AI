...
- Research Summary の次の運用方針は、外部LLMを使わず、local rule-based `ResearchBrief` へ変換してから表示すること。通常表示では provider profile の生フィールド羅列を出さず、企業概要、定量評価サマリー、良材料候補、注意材料候補、不足根拠、次に確認すべき資料、出典カードに整理する。
- `ResearchBrief` の定量評価では、取得できた PER / PBR / ROE / 売上高 / 営業利益 / 純利益 / EPS / 配当 / 時価総額などを source type と confidence 付きで表示する。取得できない主要指標は missing metrics として明示する。
- `ResearchBrief` の confidence は情報源の信頼度であり、投資判断の正しさではない。公式IR / TDnet / EDINET / 企業IRは high、Yahoo Finance / provider profile / news は medium、キーワード抽出のみは low として説明する。

...
- Investment Radarの`response_schema=radar_interpretation.v1`だけは、candidate IDと項目別`cited_evidence_ids`を持つstrict JSONをGatewayから返す。Gatewayは許可済みcontext IDだけを参照し、親SMAIがsymbol・数値・日付・助言表現のgrounding validationとdeterministic fallbackを担う

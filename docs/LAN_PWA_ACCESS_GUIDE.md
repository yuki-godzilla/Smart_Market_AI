...
自動起動または監視復旧と同時に実行されても、共通ランチャーが二重起動を防ぎます。
すでにSMAIが起動中の場合は、既存サーバーをそのまま利用します。
...
- 起動BATに`Tailscale URL`が表示された場合、そのURLをTailscale参加端末で開きます。
...
実機では、PWAを閉じた直後、5分経過後、Tailscale接続時をそれぞれ確認してください。

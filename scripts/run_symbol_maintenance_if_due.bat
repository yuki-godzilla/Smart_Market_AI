...
set "SMAI_LOG_FILE=%SMAI_LOG_DIR%\symbol_maintenance_if_due_%SMAI_RUN_ID%.log"

call :log "============================================================"
call :log "[SMAI] Symbol maintenance if-due check started."
...
call :log "[SMAI] Interval days: %SMAI_SYMBOL_MAINTENANCE_INTERVAL_DAYS%"
call :log "[SMAI] Retry cooldown hours: %SMAI_SYMBOL_MAINTENANCE_RETRY_COOLDOWN_HOURS%"
...
"%SMAI_PYTHON%" tools\symbol_maintenance_state.py begin ^
  ...
  --lock "%SMAI_LOCK_FILE%" ^
  --log-path "%SMAI_LOG_FILE%" >> "%SMAI_LOG_FILE%" 2>&1
set "SMAI_DECISION_EXIT=%ERRORLEVEL%"

if "%SMAI_DECISION_EXIT%"=="10" (
    call :log "[SMAI] Maintenance is not due. Skipping."
    exit /b 0
)
if "%SMAI_DECISION_EXIT%"=="11" (
    call :log "[SMAI] Previous failure is inside retry cooldown. Skipping."
    exit /b 0
)
if "%SMAI_DECISION_EXIT%"=="12" (
    call :log "[SMAI] Maintenance lock exists. Skipping duplicate execution."
    exit /b 0
)
if not "%SMAI_DECISION_EXIT%"=="0" (
    call :log "[ERROR] Maintenance decision failed with exit code %SMAI_DECISION_EXIT%."
    exit /b %SMAI_DECISION_EXIT%
)

call :log "[SMAI] Maintenance is due. Starting run_symbol_universe_import_all.bat..."

...
- `良材料候補` / `注意材料候補` は「どの材料を何件確認したか」「主な出典は何か」を示す。英語抜粋や provider raw fields をそのまま貼らない。
- ニュース未取得、URLなし、資料不足、低信頼などは `注意材料候補` ではなく `確認不足` に寄せる。
- `定量指標` は source type と confidence が見える小カードで表示し、取得できない主要指標は `確認不足` の警告パネルにまとめる。
...
主操作は primary button、副操作や保存操作は secondary button、download button、または expander 内に置きます。

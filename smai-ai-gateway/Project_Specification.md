...
- 現在の Assistant / context-answer 用途では、LLM の役割を説明、要約、確認観点の整理、Cockpit材料の読み解き支援に限定する。optional Tool Planner でも Gateway は action案JSONを返すだけで、実行・検証・fallback・UI採用は親SMAI側が担う。
- `SMAI LLM Factor` 用途では、LLM を最終予測器ではなく、出典付き定性材料を構造化 JSON 特徴量へ変換する provider として扱う。
...
| FastAPI API 層 | Gateway | `/health`、`/api/v1/chat`、`/api/v1/summarize`、`/api/v1/context-answer`、`/api/v1/assistant/tool-plan` を公開する。 |
...
| Cockpit LLM interpretation support | 親側MVP実装済み / Gateway task対応済み | Cockpit材料の読み解き支援 | 親SMAI側でcontext compression、validation、cache、fallback、`AI解釈メモ` UIを扱う。Gateway は `/api/v1/context-answer` の `task_type=cockpit_interpretation` として回答整理だけを担う | 親側 roadmap / [docs/prompt_policy.md](docs/prompt_policy.md) |
| Ranking LLM interpretation support | 親側MVP実装済み / Gateway strict schema対応済み | 固定済みRanking候補の読み解き支援 | 親SMAI側の明示実行時だけ `task_type=ranking_interpretation` / `response_schema=ranking_interpretation.v1` を使う。Gatewayは候補順とcitationを検証し、親側が追加grounding / cache / fallback / UIを担う | 親側 roadmap / [docs/prompt_policy.md](docs/prompt_policy.md) |
| Investment Radar Overview interpretation support | 親側MVP実装済み / Gateway strict schema対応済み | 固定済みRadar画面全体の読み解き支援 | 親SMAI側の明示実行時だけ `task_type=radar_overview_interpretation` / `response_schema=radar_overview_interpretation.v1` を使う。Gatewayはcontext hash、sector/theme/candidate関係、citation、stale market policyを検証し、親側が追加grounding / profile cache / fallback / UIを担う | 親側 roadmap / [docs/prompt_policy.md](docs/prompt_policy.md) |
| Assistant LLM Tool Planner | 親側MVP実装済み / Gateway endpoint対応済み | optional plan proposal | Gateway は `/api/v1/assistant/tool-plan` で available actions 限定の JSON plan 案を返す。親SMAI側が schema / allowlist / safety validation、deterministic fallback、`次にできること` / `確認フロー` UI採用、確認付きaction実行を担う | 親側 roadmap / [docs/api_spec.md](docs/api_spec.md) |
| SMAI LLM Factor structured extraction support | 実装済み | RAG / News / IR 由来の定性材料を構造化特徴量へ変換する補助 | Gateway 側は `/api/v1/llm-factor/generate` で provider / prompt 境界を提供済み。SMAI 本体側の `LLMFactorResult` schema / fake service / file-backed cache / deterministic backtest evaluator / broader historical fixture / validation report / Cockpit 参考表示 / Ranking 参考表示も実装済み。cache policy expansion、UI 統合は SMAI 本体側で扱う | 親側 roadmap / [docs/prompt_policy.md](docs/prompt_policy.md) |
...
| `POST /api/v1/context-answer` | `user_question`, `context`, 任意 `constraints`, 任意 `model` | `answer`, `materials`, `cautions`, `next_checkpoints`, `referenced_sections`, optional strict interpretation payload、`confidence`, `provider`, `model`, `elapsed_ms` | LLM がスコアや順位を変更せず、渡された context から説明補助だけを返すこと。`task_type=cockpit_interpretation` はCockpit、`ranking_interpretation.v1` は固定済みRanking候補、`radar_overview_interpretation.v1`は固定済みRadar overviewのcitation付き整理に限定する。 |
| `POST /api/v1/assistant/tool-plan` | `user_question`, `current_page`, `context_summary`, `available_actions`, 任意 `constraints` | `plan_type`, `user_intent`, `overall_summary`, `steps`, `safety_note`, `provider`, `model`, `gateway_status` | LLM が action を実行せず、許可済み action_id の plan 案だけを返すこと。親SMAI側の validation / fallback が最終採用を決める。 |
...
- `SMAI LLM Factor` 向けの structured extraction endpoint / prompt profile は実装済み。domain schema / deterministic fake service / file-backed cache / deterministic backtest evaluator / broader historical fixture / validation report / Cockpit 参考表示 / Ranking 参考表示は SMAI 本体側にあり、cache policy expansion、UI 統合拡張も SMAI 本体側で扱う。
...
| Cockpit LLM Interpretation | 親側MVP実装済み / Gateway task対応済み | 親SMAI側の `AI解釈メモ` が `/api/v1/context-answer` の `task_type=cockpit_interpretation` を使う。Gateway は読み解き文と材料整理を返し、親側が validation / cache / fallback / UI 表示を担う。 |
| Ranking LLM Interpretation | 親側MVP実装済み / Gateway strict schema対応済み | 親SMAI側の明示ボタンだけが `task_type=ranking_interpretation` / `ranking_interpretation.v1` を使う。Gatewayは候補順、candidate ID、citationを検証し、順位・score・Forecastは変更しない。 |
| Investment Radar Overview Interpretation | 親側MVP実装済み / Gateway strict schema対応済み | 親SMAI側の明示ボタンだけが `task_type=radar_overview_interpretation` / `radar_overview_interpretation.v1` を使う。Gatewayはcontext hash、sector/theme/candidate関係、citation、stale market policyを検証し、候補順・価格・score・Forecastは変更しない。 |
| Assistant LLM Tool Planner | 親側MVP実装済み / Gateway endpoint対応済み | 親SMAI側の `assistant.llm_planner.enabled=true` 時のみ `/api/v1/assistant/tool-plan` を使う。Gateway は JSON plan 案を返し、親側が allowlist / confirmation / unsafe wording validation と deterministic fallback を担う。 |
| SMAI LLM Factor | 親側 validation slice 実装済み / Gateway endpoint実装済み | LLM を最終予測器ではなく、source-bound qualitative feature generator として扱う。Gateway 側は `/api/v1/llm-factor/generate` で provider / prompt 実行境界を提供し、SMAI 本体側に schema / fake service / file-backed cache / deterministic backtest evaluator / broader historical fixture / validation report / Cockpit 参考表示 / Ranking 参考表示を置く。 |

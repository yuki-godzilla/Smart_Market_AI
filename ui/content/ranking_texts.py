...
RANKING_FILTER_HELP_TEXTS = {
    ...
    "period": (
        "ランキング計算に使う価格データの期間です。予測日数は取得期間から自動計算し、同じランキング内では全銘柄を共通期間で比較します。"
...
RANKING_CHART_PROFILE_TEXTS: dict[str, RankingChartProfileText] = {
    "reversal_expectation": {
        "title": "上向き兆候の根拠マップ",
        "description": "上位10件について、総合スコアと6つの構成要素を同じ尺度で比較します。このマップではデータ品質を魅力度の軸に使わず、銘柄名の横に評価可否として表示します。",
        "how_to_read": (
            "行: 上向き兆候ランキングの上位候補",
            "列: 総合、形状、予測余地、下落安全、押し目、補助品質、上向き材料",
            "明るいセルほど、その構成要素のスコアが高い",
            "評価可能・要確認・評価対象外は、投資魅力度ではなく計算材料の状態",
        ),
        "caution": "補助品質は現行式のスクリーニング・DB登録・配当確認材料で、行ラベルのデータ品質ゲートとは別です。上位でも反発や上昇を断定せず、下落理由、予測根拠、配当維持力を確認してください。",
    },
    "score_risk": {
        "title": "スコア x リスク",

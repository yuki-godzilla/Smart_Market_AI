...
| Phase N1〜N4 / Notification Platform | ⬜ **設計済み / 未着手** | 独立 `smai-notification-gateway`、親側 client、通知設定、SQLiteアプリ内履歴、通知センターを段階導入する。ntfy失敗時もアプリ内履歴を残し、本体処理を止めない。 |
...
- ⬜ **設計済み / 未着手**: Phase N1〜N4 通知基盤。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。初期MVPはアプリ内通知 + 明示設定された ntfy とし、外部送信失敗をSMAI本体へ伝播させない。
...
1. N1: 独立 `smai-notification-gateway`、contracts、NtfyChannel、Dispatcher、単体テスト
2. N2: SMAI 親側 notification client と明示的なテスト通知

...
For Research RAG / News RAG, product behavior should prioritize current external sources because freshness is central to the feature. `AI調査を更新` is the standard external-source search action for the selected symbol, currently using TDnet + Yahoo Finance as the first live source set and planned to expand to official IR, EDINET, company sites, provider profiles, and news adapters. The Cockpit Research operation card keeps this as the only always-visible action and uses `AI調査で確認すること` before fetch and `判断材料に変換しました` after fetch to summarize business overview, checked source scope, and next confirmation; lower-frequency news-only refresh, CSV, and detail-toggle controls are not part of the main path. Local documents remain important for deterministic tests, demo seeds, user-saved archives, private notes, and fallback, but they are no longer the intended primary product source. Normal checks still use fake adapters / fixtures and must not depend on live network. Fetched source text should not be retained by default; persistent archive should require a separate explicit user action.

Current Research Summary maturity work is local-first readability, not external LLM integration. A display-only `ResearchBrief` / `ResearchMetric` layer converts `CompanyResearchReport`, RAG evidence, provider profile snippets, news, and TDnet traces into a readable AI整理メモ, reading guide, quantitative summary, business overview, positive / caution materials, missing evidence, next actions, and source cards. A separate display-only `InvestmentInsight` / `InvestmentInsightBuilder` layer turns the same source-backed inputs into a `SMAI 投資判断サマリー` with a status label, confidence label, primary action, 3-line current view, good materials, caution materials, and top confirmation gaps; it does not affect Research Score, Investment Score, or ranking order. `InvestmentQuestionSummary` / `InvestmentQuestionSummaryBuilder` now adds a fixed 10-question review layer for what users want to know before reviewing a symbol: business model, sales/profit trend, profitability, outlook, growth drivers, risks, shareholder returns, valuation, news impact, and the key takeaway. The first `ResearchFactSummary` slice is implemented as a source-backed fact layer between evidence/news/provider payloads and `ResearchBrief`; it surfaces business overview, key businesses, regions / revenue drivers, checked IR / official disclosures / news, major financial metrics, earnings outlook, shareholder return / dividend policy, recent events, positive materials, caution materials, and missing items as user-readable facts rather than acquisition status. Provider raw fields and English snippets are shaped into Japanese confirmation text for the primary view; the operation card now uses `事業` / `確認済み` / `判断前に確認`, while AI整理メモ, reading guide, confirmation-point cards, source cards, Research Score, source confidence labels, and raw/source details stay behind expanders when needed. If rule-based shaping becomes insufficient, a local lightweight LLM summarizer can be considered in a later phase while keeping normal checks deterministic and falling back to the rule-based fact summary.
...
- Research Summary readability first slice: local rule-based `ResearchBriefBuilder`, display-only `InvestmentInsightBuilder`, and fixed-question `InvestmentQuestionSummaryBuilder` are implemented before any external LLM work. The primary UI order is `SMAI 投資判断サマリー` -> `投資判断で知りたいこと` -> 3-column 判断材料 (`良い材料` / `注意材料` / `まだ判断に足りない情報`) -> folded 根拠確認 (`AI整理メモ` / 読み方サマリー / `会社概要` / `確認できた事実` / `公式資料で未確認`) -> source cards / Research Score / external source traces / detail tables. The investment summary surfaces status labels such as `公式資料確認待ち`, `ニュース先行`, `材料混在`, and `定量指標不足`, plus confidence and primary action labels. The question summary always renders 10 questions, marks each answer as 根拠 高 / 中 / 低 / 不足, and keeps missing information explicit instead of filling gaps with speculation. Provider-only information is treated as neutral context. The first metric extraction uses deterministic regex / keyword rules and shows missing metrics explicitly.

...
投資レーダー内のヒートマップ見出しは `投資ヒートマップ` とします。見た目は株式ヒートマップ風に、投資カテゴリをセクター枠、関連銘柄をタイルとして表示してよい。ただし、値動きや取引量の色は市場の温度感を読むための確認材料であり、売買判断や銘柄推奨そのものに見えない説明を近くに置きます。市場指標がないカテゴリにニュース材料から補完値を出す場合は `ニュース代理` / `材料+1.2` のように、実測の値動きではなく確認用シグナルであることが分かる文言にします。

...
`smai-ai-gateway` は、LLM 通信を SMAI 本体から切り離し、SMAI 以外の会議要約アプリ、AI テスト基盤、技術文書要約ツールからも利用できる汎用 AI Gateway として整備します。
...
- LLM の役割を説明、要約、確認観点の整理に限定する。
- 数値予測、ランキング順位、売買判断、ポートフォリオ配分の決定主体にはしない。

### 2.3 対象システム

- `smai-ai-gateway`: FastAPI ベースのローカル AI Gateway。
- LLM provider: 初期はローカル Ollama。
- 利用元: SMAI 本体、会議要約アプリ、AI テスト基盤、その他ローカルツール。
...
| 要素 | 配置 / 分類 | 役割 |
| --- | --- | --- |
| SMAI 本体 | 親プロジェクト / 利用元 | Assistant context、Decision Report context、画面文脈を HTTP request として渡す将来利用元。Gateway から SMAI は import しない。 |
| 会議要約アプリ | 外部利用元 | 議事録、会議メモ、文字起こしを summarize API に渡す想定利用元。 |
| AI テスト基盤 | 外部利用元 | テスト観点生成、レビュー補助、汎用 chat に利用する想定利用元。 |
...
| `POST /api/v1/summarize` | `text`, 任意 `purpose`, 任意 `model` | `answer`, `model`, `provider`, `elapsed_ms` | 会議要約、ニュース要約、技術文書要約などに流用できること。 |
...
| [../Documents/05_Implementation_Roadmap.md](../Documents/05_Implementation_Roadmap.md) | SMAI 親モジュール側の Phase 24 roadmap。 |

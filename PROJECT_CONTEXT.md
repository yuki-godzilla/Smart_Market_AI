...
Phase 15 is the next implementation phase: define an Investment Score contract that can combine screening, forecast, risk, and data quality signals without turning outputs into buy/sell advice.
Phase 15 ????????????screening?forecast?risk?data quality ????????? Investment Score contract ?????????

## Next Good Targets / ??????

- Improve forecast chart readability with clear actual-vs-forecast styling, labels, and beginner-friendly metrics.
  ?????????????????????????????? forecast chart ??????????
- Start Phase 15 with a narrow `backend/scoring` Investment Score contract and deterministic tests.
  Phase 15 ? `backend/scoring` ? Investment Score contract ? deterministic tests ??????

...
| Phase N1〜N4 / Notification Platform | 🟨 **N1/N2実装済み / N3/N4未着手** | 独立 `smai-notification-gateway` と親側の安全な `notification_client` / 明示テスト通知関数を実装。設定UI、既存イベント接続、SQLiteアプリ内履歴、通知センターは未実装。 |
...
- 🟨 **N1/N2実装済み / N3/N4未着手**: Phase N1〜N4 通知基盤。独立gatewayと親側client境界、明示テスト通知関数まで実装済み。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。設定UI、既存イベント接続、履歴UIはまだ行わない。
...
3. N3: ntfy ON / OFF、server URL、秘匿 topic、severity threshold、quiet hours
4. N4: SQLite 通知履歴、未読 / 既読 / archive、通知センター、簡易 filter

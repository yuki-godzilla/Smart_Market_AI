...
| UX-025 | Investment News dashboard | 投資ニュース画面を開く | サイドメニューから `投資ニュース` を開き、ニュースストリーム、加熱テーマ、カテゴリ別ニュースレーン、関連銘柄導線を確認する | 市場ニュースの温度感と確認候補を把握し、気になる材料は `銘柄コックピット` で深掘りする入口だと分かる。Investment Score / Research Score / Ranking順位は変わらない | ニュースが売買推奨や銘柄推奨に見える、加熱テーマが投資判断そのものに見える | Product Safety | High | In review | Phase 22.x 初期MVP実装済み。禁止表現、URL、freshness、source type、関連銘柄 handoff は自動テストで確認。実画面smokeで表示密度と遷移を確認する |

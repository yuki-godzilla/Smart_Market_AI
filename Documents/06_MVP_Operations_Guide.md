...
- ランキング結果画面では、選択中の `並べ替え条件` に合わせて重視ポイント、メインチャート、上位カードの主指標、詳細テーブルの先頭列、`並べ替え理由` / `確認ポイント` が切り替わる。例: `上昇気配重視` では `上昇気配`、`下降警戒`、`方向スコア`、`予測変化率`、`方向一致` を前面に出す。
- `上昇気配` / `下降警戒` は、予測モデルの上向き/下向き本数だけではなく、各モデルの予測リターン幅を0〜100点化し、walk-forward方向精度に基づく控えめなモデル重みを掛けて合成する。モデル間の開きは直接加点せず、方向スコアを中立へ寄せる信頼度調整として扱う。ランキングは売買推奨ではなく、深掘り候補の比較優先度として扱う。
...
- 常設の詳細条件パネル
  ...
  - `取得期間` の `?` help では、短期は直近の値動き、中期は数週間のトレンド、長期は安定性確認に使うことを説明

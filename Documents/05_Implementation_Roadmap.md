...
- `EDINETResearchAdapter` provides optional official securities-report metadata/source links when `EDINET_API_KEY` is configured and returns no payloads without the key. `TDnetResearchAdapter` provides official timely-disclosure source links for Japanese listed-company IR, and `YahooFinanceResearchAdapter` continues to provide provider profile and recent news payloads. Tests inject fake HTTP / JSON / ticker factories, so no live EDINET, TDnet, or Yahoo call is required in normal checks.
...
- R8: External Source Adapter。EDINET optional metadata/link adapter と TDnet は初期 slice 実装済み。IR site / news などを `AI調査を更新` の標準 source adapter として広げ、通常 checks は fake adapter / fixture で network 非依存にする。

...
- Direction Signal は「上がる銘柄/下がる銘柄」ではなく、「上昇気配」「下降警戒」「深掘り候補」として表現する。`上向き兆候` は Direction Signal に混ぜず、下落・調整・横ばいから上向きに変わる兆しを確認する独立探索軸として扱う。
- Forecast agreement は主表示ではなく、モデル一致度の補助指標として扱う。
- Risk は「安全/危険の絶対判定」ではなく、「価格変動、制約違反、確認ポイント」として表現する。
- 投資レーダーの `本文に出た銘柄`、`SMAI推測候補`、`市場背景の確認` は由来が異なるため同じラベルや色で混ぜない。`確認優先度` は次に根拠を確認する順序であり、魅力度・期待収益・おすすめ度・順位として説明しない。
- 投資レーダーのAI表示は `AIによる根拠整理` とし、根拠ID、鮮度、未確認事項を近くに示す。根拠が不足する場合は `この根拠だけでは判断できません` と表示し、`AIの推奨`、`有望`、`買い候補` のような表現は使わない。
- 投資レーダーのニュース見出しは `HEADLINE FLOW` のような動きの補助表現を使ってよい。最大12件のプールから3件ずつ表示する場合も、正確な公開時刻と「8秒ごとに切替」を近くに示し、リアルタイム配信やニュースの自動更新を暗示する `速報`、`常時更新` といった未保証の表現は使わない。
...
- 通常アクセスのラベルは `SMAI接続URL` に統一し、LAN用・Remote用・VPN用などの
  接続先別ラベルを並べない。
- 補足は `このURLはLAN内・外出先で共通です。接続する端末でTailscaleを起動してください。`
  を基本とする。
- `localhost` はサーバーPC内の確認用途だけに使い、他端末へ共有するURLとして表示しない。
...
検索の補助情報は `検索方式: ハイブリッド検索（キーワード＋ローカル意味検索） / 根拠: N件 / 資料: N件` のように表示し、検索方式や件数を投資魅力度・確信度・売買判断へ読み替えさせない。意味検索の候補がない場合は、`キーワード根拠を優先します` と短く明示する。
ニュースURL表示は `外部参照ソース` と詳細データに実装済みです。`最新ニュース・開示サマリー` 周辺では、URL付きニュース・開示がある場合は `ニュース・開示の出典を表示（URL付きN件）` を初期展開して確認できるようにし、ニュース専用URLが無い場合も外部参照ソース側に公式資料・provider URLがある可能性を案内します。ニュース/開示領域は `Market Intelligence` として通常カードから分け、URL付き項目はカード全体クリックで開ける news-report-row UI にします。横長カードにせず、本文max-width、右側アクション領域、大画面2カラム、1件目featuredで視線移動を抑えます。配色は深いネイビーを基調に、通常ニュースはシアン、IR/開示はバイオレット、注目材料はアンバーゴールド、リスク材料はソフトレッド、その他はブルーグレーの控えめなネオン差分で統一します。
`投資ヒントとなるニュース` は、URL付きの一般ニュースだけを切り出すカードです。Google News RSS や Yahoo Finance news 由来の一般ニュースを対象にし、TDnet、企業IR、EDINET、Yahoo Finance profile などの source trace と混ぜません。IR/開示の根拠カードとは別系統の `注目材料 Top 3` ヘッドラインUIにし、タイトル、公開日、鮮度、出典、材料分類、確認観点、短い要約、種別アクセント、右側の外部リンク導線を優先します。raw URLは本文表示せず、カード全体をクリックして外部URLを開く導線にします。href、`target="_blank"`、`rel="noopener noreferrer"` は維持します。
...
- `元記事を見る ↗`
- `TDnetで見る ↗`
- `投資ヒントとなるニュースを外部ニュースの見出しだけで整理します。気になるカードをクリックして、本文と一次情報を確認してください。`
- `公開日`
- `鮮度`
- `元記事を見る`
- `TDnetで見る`
...
8. `投資ヒントとなるニュース`
9. `ニュース・開示の出典を表示`
10. `Research Score（根拠資料の確認材料）を表示`
11. `外部参照ソースを表示`
12. `詳細データ`
...
| `上向き兆候` | 押し目反発、底打ち接近、横ばい上放れ、蓄積上昇準備と下落安全性を整理する独立指標 | 「買い」「底打ち確定」と断定せず、下落理由を深掘りする優先度として説明する |

## 上向き兆候 / 本気分析モード 文言方針

旧「反転期待」はユーザー向けには「上向き兆候」と表記する。上昇気配はすでに上向きの強さ、上向き兆候はまだ大きく上がっていない銘柄の押し目反発・底打ち・横ばい上放れなどを評価する。

使用する文言: 上向き兆候、上向き兆候スコア、上向き兆候マップ、押し目反発待ち、底打ち接近、横ばい上放れ候補、蓄積上昇準備、売られすぎ反発狙い、落ちるナイフ注意、配当罠注意、確認価値あり、下落理由を確認、配当安全性を確認、予測信頼度、モデル見解のばらつき。

避ける文言: 買い、買い時、今がチャンス、底打ち確定、反転確定、上昇確定、必ず上がる。

本気分析モードの表示名は `本気分析モード（AI材料分析つき）` とし、「上位候補についてニュース・開示・IRをAIで確認し、悪材料・好材料・配当罠を補助評価します。通常より時間がかかります。」と説明する。AI材料分析は買い推奨ではなく深掘り確認の補助情報である。
...
独立した市場ニュース画面名は `投資レーダー` とし、ニュース種別やカード見出しの一般語として `投資ニュース` / `関連ニュース` を使います。
投資レーダーの既定タブは `市場レーダー` とし、`今日のレーダー` と独立した `市場ヒートマップ` に分けません。ヒートマップは `セクター` / `業種` / `注目ニュース` の分類を切り替え、関連銘柄を実価格タイルとして表示します。タイルには企業名とシンボル、正確な騰落率、`本文` / `テーマ推測`、根拠件数を併記し、企業名を主表示、シンボルを補助表示として扱います。騰落率は `▲ 上昇 +x.xx%` / `▼ 下落 -x.xx%` の一体表示とし、上昇は緑、下落は赤、横ばいは中立色で示します。小さいタイルでは短いシンボル、矢印、正確な値を優先し、補助語を省略しても方向や対象を取り違えないようにします。タイルクリックは該当銘柄の `銘柄コックピット` に移動する導線とし、ニュース画面だけで判断を閉じない流れにします。面積は絶対騰落率、色は上昇・下落方向とし、期間、取得元、価格基準日時、最低表示面積を凡例へ併記します。期間は `20営業日（約1か月）` に固定して選択UIを置かず、手動操作は `今すぐ更新` とします。ニュース根拠量や材料種別を、`材料+1.2` のような符号付き補完値、実測のような値動き・取引量、好悪色へ変換してはなりません。
`ニュース代理`タイルの面積は転載・重複を除いた根拠記事数、色は鮮度として凡例に明記し、独立出典数は補助表示にする。`本文に出た` / `テーマ関連`をラベルで併記し、個別タイルの価格方向を示唆しない。初期表示は最大3テーマ・各最大3タイルとし、残りは `ほか Nテーマを見る` で明示的に展開する。
投資レーダーの候補カードでは `材料種別` と記事内容の方向を混同しない。別の根拠で検証できない限り `好材料`、`注意材料`、`混在` を材料カテゴリから自動表示せず、`方向未確認` とする。通常の確認操作は `根拠資料を確認` とし、`RAG` / `ローカルRAG` は技術詳細内だけに表示する。初期候補は由来別に絞り、`あと N 件を見る`、`先に確認` / `次に確認` / `必要に応じて`、`条件を解除` のように探索状態が分かる文言を使う。
ニュースは市場や業界の動きを確認し、銘柄選定や投資判断の参考にするものとして表示します。
Cockpit Research Summary の `投資ヒントとなるニュース` はURL付き一般ニュース専用です。カードには `タイトル`、`公開日`、`鮮度`、`出典`、短い `要約` を置き、カード全体をクリックしてニュース本文を開けるようにします。ニュースが売買推奨やランキング順位の材料そのものに見えないよう、長い解説やraw URLは通常カードから外します。`最新ニュース・開示サマリー`、`投資ヒントとなるニュース`、`ニュース・開示の出典` は `Market Intelligence` / news-report-row UI として通常カードから分け、ニュース、IR/開示、重要材料、リスク材料を上品なネオン金融ダッシュボード調の背景・枠・バッジ・hoverで区別します。本文を横に伸ばしすぎず、右側に source 種別、日付、外部リンク導線を置きます。TDnetや公式IRは `ニュース・開示の出典` と `外部参照ソース` 側で扱います。
...
通知は「何が起きた」「なぜ確認したいか」「SMAI上の変化」「次に見ること」を短く整理し、Cockpitや投資レーダーなどへつなぐ判断補助カードとする。
...
お気に入り追加以降の騰落は投資成績ではなく、監視開始時点からの参考変化として表示する。通知CTAは画面遷移または確認導線とし、AI調査、外部取得、レポート作成、注文を自動実行しない。

重要度は色だけで表現せず、icon、severity label、理由を併記する。critical/riskはレッド、重要材料はゴールド/オレンジ、通常の市場・ニュースはシアン/ブルー、systemはブルーグレーを基本とする。

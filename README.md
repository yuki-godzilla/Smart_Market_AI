...
- 現在の MVP を超える screening、forecasting、高度な reporting workflow
- `SMAI_CONFIG_FILE` 以外の環境変数設定

`yahoo` / `polygon` は設定値として予約されていますが、現在は `allow_external_providers: true` の明示 opt-in がない限り拒否されます。
opt-in しても provider 本体はまだ未実装のため、現時点では外部 API へ接続しません。

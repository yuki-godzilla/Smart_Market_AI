...
- Low-cost Assistant Streamlit / Agentic first slices
  ...
  - Side menu includes `SMAIアシスタント`, a dedicated conversation workspace with SMAIナビ greeting, 6 guided intent cards, 240-character limited free text, session-local history, visible material status, centered single-column chat layout, compact per-response actions, pseudo-streaming answer display, and Markdown memo export. Free text is routed through a rule-based Intent Router and read-only `Assistant Tool Layer`; executed checks are shown as `実行した確認`.
...
- Assistant の opt-in live Gateway smoke、長い会話履歴、参照文脈の本格拡張は Phase 24 後続範囲。Streamlit の floating `SMAI Copilot` question-panel、専用 `SMAI Copilot` workspace / limited free-text first slice、SMAI 親側の opt-in Gateway HTTP client wiring は実装済み
...
SMAIアシスタントの自由会話 `free_chat` は、体感速度を優先する軽量経路です。SMAI 親側では銘柄特定 tool、RAG、news、長い会話履歴、銘柄固有 context を送らず、Gateway には短い user question と最小 context だけを渡します。Gateway 側は `free_chat` を 10 秒 / 120 tokens の短い runtime policy で処理し、単純な挨拶や挨拶で始まる短文は local fast path で即時応答します。アシスタント画面は `header -> material chips -> initial quick cards or chat thread -> bottom composer` を基本構造とし、主要セクションを SMAI 共通の 1320px content lane、会話本文を 1040px chat lane に揃えます。モデル選択 UI は composer 内で入力欄の左に置きます。

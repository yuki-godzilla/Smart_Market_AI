...
Current Research Summary maturity work is local-first readability, not external LLM integration. A display-only `ResearchBrief` / `ResearchMetric` layer converts `CompanyResearchReport`, RAG evidence, provider profile snippets, news, and TDnet traces into a readable AI整理メモ, quantitative summary, business overview, positive / caution candidates, missing evidence, next actions, and source cards. Provider raw fields are filtered out of the normal brief view and remain detail-only when needed.

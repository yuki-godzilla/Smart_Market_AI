...
- Research Summary readability first slice: local rule-based `ResearchBriefBuilder` and display-only `InvestmentInsightBuilder` are implemented before any external LLM work. The primary UI order is `SMAI 投資判断サマリー` -> 3-column 判断材料 (`良い材料` / `注意材料` / `まだ判断に足りない情報`) -> folded 根拠確認 (`AI整理メモ` / 読み方サマリー / `会社概要` / `確認できた事実` / `公式資料で未確認`) -> source cards / Research Score / external source traces / detail tables. The investment summary surfaces status labels such as `公式資料確認待ち`, `ニュース先行`, `材料混在`, and `定量指標不足`, plus confidence and primary action labels. Provider-only information is treated as neutral context. The first metric extraction uses deterministic regex / keyword rules and shows missing metrics explicitly.

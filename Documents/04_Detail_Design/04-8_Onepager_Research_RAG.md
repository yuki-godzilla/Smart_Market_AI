...
- Phase 21.6 は `AI調査を更新` の標準処理として、EDINET optional metadata/link adapter + TDnet + Yahoo Finance 初期 slice 実装済み。次は 企業IR site / provider profile 拡張などの外部資料取得 adapter を追加する候補とする。
...
* 外部取得は `AI調査を更新` の標準 source とする。EDINET optional metadata/link adapter + TDnet + Yahoo Finance は初期 slice 実装済みで、IR RSS / 企業IR / News API 等は adapter として段階的に追加する。

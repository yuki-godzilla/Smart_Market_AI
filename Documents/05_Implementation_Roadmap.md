...
10. SBI / NISA / future 投信 metadata source import. 部分完了。`--source-profile`、JPX stock/ETF profile、SBI US stock/ETF seed、NISA eligibility update profile/seed、mutual fund seed、投信 metadata columns の import path は追加済み。JPX 東証上場銘柄一覧 raw file から国内株 source CSV を作る `build_symbol_universe_source.py --source-kind jpx_listed_stock`、JPX 国内 ETF / ETN source CSV を作る `--source-kind jpx_etf`、SBI米国株 / 米国ETF・海外ETF raw file から source CSV を作る `--source-kind sbi_us_stock` / `sbi_us_etf`、NISA raw file から制度 metadata source CSV を作る `--source-kind nisa_eligibility` も追加済み。JPX NISA 成長投資枠 ETF/ETN Excel は 27件を `symbol_universe.csv` に反映済み。MVP ranking は stock / ETF のみ対象にし、mutual fund seed は future extension として保持する。次は公式 ETF 全銘柄一覧の Excel/CSV 化、SBI米国株/海外ETF source の拡張、NISA一覧の継続更新を進める。

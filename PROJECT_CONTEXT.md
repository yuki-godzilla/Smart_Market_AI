...
- Research Summary readability first slice: local rule-based `CompanyResearchSummaryBuilder`, `ResearchBriefBuilder`, display-only `InvestmentInsightBuilder`, and fixed-question `InvestmentQuestionSummaryBuilder` are implemented before any external LLM work. The primary UI order is `企業リサーチサマリー` -> `定量情報サマリー` -> `IR情報サマリー` -> `最新ニュースサマリー` -> `AI読み取りメモ` / `企業理解の確認ポイント` -> 3-column company-understanding cards (`確認できた情報` / `注意して読む情報` / `不足している情報`) -> folded 根拠確認 -> source cards / Research Score / external source traces / detail tables. The company summary keeps business overview, segments, regions, scale, IR availability, news, and missing critical items explicit. Provider-only information is treated as neutral context. The first metric extraction uses deterministic regex / keyword rules and shows missing metrics explicitly.

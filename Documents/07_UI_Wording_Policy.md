...
投資レーダー内のニュース概観の見出しは `ニューステーマ`、実価格の比較は `値動き注目マップ` とします。両者を同じヒートマップとして混同しません。投資カテゴリをテーマ枠、関連銘柄をタイルとして表示してよい。タイルには企業名とシンボルを併記し、企業名を主表示、シンボルを小さな補助タグとして扱います。長い名称は省略しつつ hover title で確認できるようにします。ニューステーマのタイルクリックは該当銘柄の `銘柄コックピット` に移動する導線とし、ニュース画面だけで判断を閉じない流れにします。`値動き注目マップ` は面積を絶対騰落率、色を上昇・下落方向とし、正確な騰落率、期間、取得元、価格基準日時を併記します。`ニュース代理` は `方向未確認` としてニュース上の注目度だけを示し、`材料+1.2` のような符号付き補完値、実測のような値動き・取引量、材料種別だけによる好悪色を表示してはなりません。

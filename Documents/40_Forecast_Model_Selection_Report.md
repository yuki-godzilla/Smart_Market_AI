...
1. `horizon-conditioned conservative calibration`のevaluation-only実装と既存cohort評価は完了。
2. overall gateは通過したが20日downtrendのsubgroup gateは未通過。runtimeへ接続しない。
3. 同profileを変更せず、新規symbol / 新期間のwalk-forward cohortでdowntrend再現性を確認する。
...
6. 新期間・新symbolのsealed auditを通過した場合だけCockpit / Ranking接続を検討する。

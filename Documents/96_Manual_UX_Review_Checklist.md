...
| UX-014a | Research Summary / Brief | ローカルAI整理メモを読む | `AI調査を更新` 後に Research Summary 冒頭を確認する | AI整理メモ、読み方サマリー、`会社概要` / `確認できた事実` / `公式資料で未確認` が順に読め、provider raw fields が通常表示に出ない。事業、確認済みIR / 公式資料、主要数値、直近イベント、未確認項目が source-backed fact として表示される | 生データ羅列で何を確認すべきか分からない、取得状態だけで中身が分からない、またはAIが投資判断したように見える | UX Readability | High | In review | 情報過多にならず、主表示と折りたたみ詳細の境界が分かるか実画面で確認。良材料/注意材料候補、出典カード、Research Score、外部参照ソース、詳細表は必要時に開く情報として分ける |

...
- `EDINETResearchAdapter` provides optional official securities-report metadata/source links when `EDINET_API_KEY` is configured and returns no payloads without the key. `TDnetResearchAdapter` provides official timely-disclosure source links for Japanese listed-company IR, `CompanyIRSiteResearchAdapter` discovers a company official IR page from official website metadata, and `YahooFinanceResearchAdapter` continues to provide provider profile and recent news payloads. `ExternalStockNewsFetchService` can also normalize external news adapter output directly into `StockNewsEvidence`, with URL dedupe, freshness warnings, and an explicit network opt-in gate. Tests inject fake HTTP / JSON / ticker/news factories, so no live EDINET, TDnet, company IR, Yahoo, or news-provider call is required in normal checks.

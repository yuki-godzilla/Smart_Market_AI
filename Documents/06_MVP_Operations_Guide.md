...
- 価格・予測チャート
- `上昇気配・下降警戒`: ランキングと同じ2指標を1銘柄の深掘り用に表示し、`上昇気配`、`下降警戒`、`予測変化率`、モデル方向一致、予測のばらつきを確認する。売買推奨ではなく比較・確認材料として扱う。
...
- ランキング結果画面では、選択中の `並べ替え条件` に合わせて重視ポイント、メインチャート、上位カードの主指標、詳細テーブルの先頭列、`並べ替え理由` / `確認ポイント` が切り替わる。例: `上昇気配重視` では `上昇気配`、`下降警戒`、`予測変化率`、`方向一致` を前面に出す。

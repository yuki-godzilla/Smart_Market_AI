...
- Feature Store Lite: started with reusable `FeatureSnapshot` rows and Streamlit preview for return, momentum, volatility, drawdown, ADV, data completeness, missing metadata, and data-quality judgement / Feature Store Lite は再利用可能な `FeatureSnapshot` 行と、return、momentum、volatility、drawdown、ADV、data completeness、missing metadata、data-quality judgement の Streamlit preview から着手済み

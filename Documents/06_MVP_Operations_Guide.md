...
- Cockpit `AI解釈メモ` は Phase 28-A、Ranking `Ranking AI解釈（参考）` は Phase 28-B MVP として実装済み。Ranking側は既定disabledかつ明示実行のみで、順位・score・Forecastを変更しない。Radar / News / Decision Report への展開、Assistant からの画面説明連携、Decision Report への自動挿入は後続範囲

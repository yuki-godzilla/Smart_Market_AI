...
| UX-014b | Research Summary / News source URL | ニュース・開示URL導線を確認する | `AI調査を更新` 後に `最新ニュース・開示サマリー` と `ニュース・開示の出典を表示（URL付きN件）` を確認する | URL付き出典がある場合は初期展開され、URL付きニュース、TDnet、企業IR、EDINET、Yahoo Finance が `Market Intelligence` / news-report-row UI で表示される。大画面では2カラム、1件目featured、本文max-width、右側アクション領域で横長表示が抑えられる。カード全体クリックで href が開き、`target="_blank"` / `rel="noopener noreferrer"` が維持され、raw URL文字列は本文表示されない。ニュース専用URLが無い場合も、外部参照ソース側の公式資料・provider URLを確認できる可能性が伝わる | URL表示が未実装に見える、ニュースと公式開示が混同される、provider raw fields や取得本文が通常UIに出る、クリック可能性が分からない、横長で読みにくい | UX Readability | High | In review | Market Intelligence layout polish slice 実装済み。実画面回帰で国内株 / 米国株 / ETF / source no-op / 大阪ガスを確認する |

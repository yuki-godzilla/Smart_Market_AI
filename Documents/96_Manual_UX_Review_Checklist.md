...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UX-001 | Symbol Cockpit | 1銘柄を初めて確認する | 銘柄を選び、期間を選択し、価格データを取得する | 価格、特徴量、Forecast、Investment Score、注意点の関係が順番に理解できる | 情報量が多く、最初に何を見るべきか分からない | UX Confusion | High | In review | 2026-06-03に価格・予測、評価の内訳、読み分け表の文言を補強し、Chrome headless + mock 7203.T 取得後画面で確認。live provider は継続確認 |
| UX-002 | Symbol Cockpit | Investment Score を読む | スコア、内訳、警告、メモを確認する | 売買判断ではなく、比較・分析用の補助スコアとして読める | 高スコアが「買うべき」に見える | Product Safety | High | In review | 2026-06-03にカードhelp / 評価帯 / 内訳表へ非助言・比較分析用の読み方を追加し、Chrome headless + mock 7203.T 取得後画面で確認 |
...
| UX-016 | Forecast | Forecast chart を読む | 複数モデルの予測線、実績線、評価指標を確認する | 予測は不確実な参考情報だと分かる | 予測線が確定未来に見える | Product Safety | High | In review | 2026-06-03に価格・予測captionと予測評価の確認ポイントを「モデル間の見方 / 不確実性 / 将来保証ではない」表現へ補強し、Chrome headless + mock 取得後画面で確認 |
| UX-017 | Forecast | Forecast agreement を読む | Investment Score の Forecast agreement を確認する | モデル間の見方の近さとして理解できる | 的中率や予想の正しさに見える | Wording Risk | Medium | In review | 2026-06-03に評価内訳へ `予測・モデル一致` 行を追加し、的中率や将来保証ではないことをChrome headless + mock 取得後画面で確認 |
| UX-018 | Risk | Risk score を読む | Ranking / Cockpit / Rebalance で risk 表示を確認する | 価格変動や制約違反の確認材料だと分かる | リスクが低ければ安全と誤解される | Wording Risk | High | In review | 2026-06-03にリスク確認のカードhelp / 評価帯 / 読み分け表へ安全保証ではない説明を追加し、Cockpit mock 取得後画面で確認。Ranking / Rebalance は継続確認 |
...
| UX-025 | Investment News dashboard | 投資レーダー画面を開く | サイドメニューから `投資レーダー` を開き、ニュースストリーム、加熱テーマ、カテゴリ別ニュースレーン、関連銘柄導線を確認する | 市場ニュースの温度感と確認候補を把握し、気になる材料は `銘柄コックピット` で深掘りする入口だと分かる。Investment Score / Research Score / Ranking順位は変わらない | ニュースが売買推奨や銘柄推奨に見える、加熱テーマが投資判断そのものに見える | Product Safety | High | In review | Phase 22.x 初期MVP実装済み。禁止表現、URL、freshness、source type、関連銘柄 handoff は自動テストで確認。実画面smokeで表示密度と遷移を確認する |

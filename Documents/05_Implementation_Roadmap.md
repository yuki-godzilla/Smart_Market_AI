...
- The current implementation still has an explicit `allow_network=True` backend safety gate and a separate Cockpit `外部資料取得（明示許可）` UI. Next UI work should remove the separate permission panel and make `AI調査を更新` the standard external source search action while retaining fake-adapter tests and backend safety boundaries.

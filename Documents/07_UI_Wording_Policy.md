...
投資レーダー内の市場概観の見出しは `今日のテーママップ` とします。投資カテゴリをテーマ枠、関連銘柄をタイルとして表示してよい。タイルには企業名とシンボルを併記し、企業名を主表示、シンボルを小さな補助タグとして扱います。長い名称は省略しつつ hover title で確認できるようにします。タイルクリックは該当銘柄の `銘柄コックピット` に移動する導線とし、ニュース画面だけで判断を閉じない流れにします。`市場データ` は取得済みの価格・取引量を明示し、`ニュース代理` は `方向未確認` としてニュース上の注目度だけを示す。ニュース代理に `材料+1.2` のような符号付き補完値、実測のような値動き・取引量、材料種別だけによる好悪色を表示してはならない。

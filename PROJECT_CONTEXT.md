...
For Research RAG / News RAG, product behavior should prioritize current external sources because freshness is central to the feature. `AI調査を更新` should evolve into the standard external-source search action for the selected symbol, using sources such as official IR, EDINET / TDnet, company sites, provider profiles, and news adapters as they become available. Local documents remain important for deterministic tests, demo seeds, user-saved archives, private notes, and fallback, but they are no longer the intended primary product source. Normal checks still use fake adapters / fixtures and must not depend on live network. Fetched source text should not be retained by default; persistent archive should require a separate explicit user action.

...
Current Research Summary maturity work is local-first readability, not external LLM integration. A display-only `ResearchBrief` / `ResearchMetric` layer converts `CompanyResearchReport`, RAG evidence, provider profile snippets, news, and TDnet traces into a readable AI整理メモ, quantitative summary, business overview, positive / caution materials, missing evidence, next actions, and source cards. Provider raw fields and English snippets are shaped into Japanese confirmation text for the primary view; material cards show source rank and source confidence, confirmation gaps use beginner-friendly wording, and raw/source details remain behind expanders when needed.

...
| Decision Report | あとから見返すために、価格、予測、根拠資料、注意点、確認ポイントを整理するレポート。 | 投資推奨書、注文指示書 |
...
| 領域 | 実装状態 | メモ |
| --- | --- | --- |
| MarketData | 実装済み | default `yahoo` / test-offline `mock` / `csv` 経路 |
...
- ローカルPDF/Markdown/Text ingestion は deterministic fixture / archive / fallback として維持する。
- ユーザー向けの `AI調査を更新` では、EDINET securities-report metadata/link（`EDINET_API_KEY` 設定時のみ live call、未設定時 no-op）、TDnet timely disclosure、企業IR adapter（後続）、provider profile、news などの外部最新 source を優先する。
- Research Summary は local rule-based `ResearchBrief` で、AI整理メモ、定量指標、企業概要、良材料候補、注意材料候補、不足根拠、次の確認資料、出典カード、詳細データに分けて表示する。
- `ResearchFactSummary` は、取得状態ではなく、事業概要、主要事業、確認済みIR / 公式資料、主要定量指標、直近イベント、良材料候補、注意材料候補、未確認項目を source-backed fact として `ResearchBrief` に渡す。
- 外部LLM要約は future / optional とし、現行の読みやすさ改善 slice では必須にしない。
...
- 実装済み土台: ローカルPDF/Markdown/Textの登録、chunk化、keyword search、テンプレート要約、query expansion、structured extraction、grounded answer、Research Score 初期 slice、EDINET optional metadata/link + TDnet + Yahoo Finance 外部取得の初期 slice。
- 将来拡張: 企業IR / IR RSS / News adapter、ResearchFactSummary の抽出対象拡張、ranking order の明示 opt-in 統合、optional LLM summary。
- 制約: RAG単体で売買推奨を行わず、根拠と注意点を Decision Report / Assistant / Investment Score に渡す。
- News RAG: 現在は `銘柄コックピット` の選択銘柄に限り、title / URL / source / published_at / summary / investment_viewpoint / sentiment_for_investment / freshness_status を表示する補助情報として扱う。市場全体の `投資ニュース` 画面、News Score 化は後続候補。ニュース単独で ranking order を変更しない。
...
主要探索軸は `AI総合 → 上昇気配 → 上向き兆候 → 下降警戒` の順で表示する。上向き兆候は旧 `reversal_expectation_score` のユーザー向け名称で、内部keyは互換目的で維持できる。

形状は、押し目反発待ち、底打ち接近、横ばい上放れ候補、蓄積上昇準備、売られすぎ反発狙い、上昇済み・兆候薄め、落ちるナイフ注意、配当罠注意、データ不足・要確認に分類する。

### Forecast model improvement workflow

1. point-in-time walk-forward評価を行う。
2. horizon、market、asset type、regime別に集計する。
3. 得意・不得意とerror caseを記録する。
4. consensus weighting案を作る。
5. 上向き兆候への寄与とランキング前後差を確認する。
6. 新規モデルの必要性を判断する。

### 本気分析モード

初期値OFFとし、ONの場合だけ通常ランキング完了後の上位候補をAI材料分析する。初期段階はバッジ・警告のみとし、性能評価試験通過後だけ上位候補内で軽く補正する。処理中・完了・失敗を表示し、失敗しても通常ランキングを残す。

### 上向き兆候バックテスト

評価日時点までのデータだけでスコアを計算し、20・60・120営業日後return、最大下落、benchmark/excess return、success/false positive flagを確認する。10スプリントの実銘柄確認と既知例・自動抽出例を併用する。

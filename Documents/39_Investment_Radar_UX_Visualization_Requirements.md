...
作成日: 2026-07-13
状態: P0 実装済み / P1 一部着手（後続 UI/UX スプリントの正本）
...
- P0 は、材料種別と方向の分離、候補数の上限と展開、探索条件の統合、到達可能な詳細ダイアログ、レスポンシブ修正として実装済みである。`今日の確認ポイント`、コンパクトな `今日のテーママップ`、確認トリアージは P1 の土台として先行実装した。テーママップ初期表示は、元の一覧性を保つ3テーマ × 各最大3タイルとし、残りテーマは明示的な展開へ置く。
...
- テーマタイルから候補キューへの連動、根拠タイムライン／根拠ルートマップ、前回 snapshot 差分、Decision Trail 連携は未実装であり、P1 後半以降の対象として残す。現行タイルの選択は該当銘柄の `銘柄コックピット` 導線であり、候補一覧を自動で絞り込まない。
...
画面は「市場の概観 → 確認する候補 → 選択した根拠 → 次の操作」の順に読む。長い候補一覧を最初の画面の主役にしない。
...
- `tests/test_news_dashboard_service.py`、`tests/test_ui_news_view.py`、`tests/test_ui_news_streamlit_page.py`、responsive smoke — 意味・操作・画面幅の回帰

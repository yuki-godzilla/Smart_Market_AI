...
- `symbol_universe.csv` への source seed 反映
  ...
  - JPX NISA 成長投資枠 ETF/ETN source として 28件を追加/更新済み
  - SBI US stock official HTML source として 4,330行を source 化し、新規6件を追加済み
  - SBI US ETF official HTML source として 612件を source 化し、新規9件を追加済み
  - SBI最新一覧から消えた米国株19件・米国ETF5件を `not_tradable` / `is_sbi_supported=false` として保持
  - ETF 1,046件は JPX / IMAJ / SBI source 照合により `nisa_category=unknown` を解消済み
...
| `sbi_availability` | SBI最新一覧から消えた既存銘柄の取扱可否更新 source | 既存銘柄の `tradability`, `is_sbi_supported`, metadata fields だけを更新 |
...
| `quality_review` | provider値の目視レビュー結果 source | 既存銘柄の `data_quality`, metadata fields だけを更新 |
...
`sbi_availability` は `--update-existing` と組み合わせる前提で、SBI最新一覧から消えた既存銘柄を削除せず `tradability=not_tradable`, `is_sbi_supported=false` として保持する。SBI公式HTMLは同一 ticker の重複や旧表記が混在する場合があるため、通常は既存銘柄の名称を一括上書きしない。
...
`quality_review` は、provider 再取得後も極端な値が残る場合などに、値そのものを置き換えず `data_quality=WARN` / `BLOCK` だけを更新するために使う。
...
2026-05-26 時点の `symbol_universe.csv` は 9,197件です。内訳は stock 8,087件、ETF 1,046件、REIT 58件、投信 4件、ADR 2件です。MVP ranking universe はこのうち `stock` / `etf` のみを対象にします。
`nisa_eligibility_seed.csv` から 31件の NISA metadata を反映済みです。加えて、国内株 3,747件と米国株 4,334件を成長投資枠対象として backfill し、JPX NISA 成長投資枠 ETF/ETN Excel から 28件、IMAJ NISA 成長投資枠 listed-fund Excel から既存 289件の NISA metadata を反映済みです。レバレッジ / インバース ETF は `nisa_category=none` へ整理します。IMAJ source に含まれるインフラファンド等 5件は現行 MVP の候補マスタには未登録のため、`nisa_eligibility` の update-only failure として manifest に残します。2026-05-26 の SBI最新一覧差分では、米国株19件・米国ETF5件を削除せず `not_tradable` / `is_sbi_supported=false` として保持しています。
`data/marketdata/symbol_universe_metadata_coverage.json` は現在の ranking metadata 充足状況です。JPX/Yahoo-covered stock rows と SBI公式米国株HTMLから追加した銘柄は、確認済み source CSV または明示 opt-in metadata refresh で `PER` / `PBR` / `ROE` / `配当利回り` を段階的に補完します。2026-05-26 時点では株式 8,087件のうち、`配当利回り` 8,033件、`PBR` 7,630件、`ROE` 7,466件、`PER` 7,457件が埋まっています。ETF は `指数` が 1,046/1,046 件、`信託報酬/経費率` が 1,025/1,046 件、`複雑さ` が 1,046/1,046 件まで埋まっています。SBI米国ETFの Yahoo coverage 失敗は `tools/analyze_yahoo_coverage_failures.py` で原因別に保存し、2026-05-22 時点ではレバレッジ除外3件、`yahoo_symbol` mapping 済み11件に整理済みです。2026-05-26 の数値異常チェックでは、対象16銘柄を Yahoo で再取得し、極端値が残ったため `data_quality=WARN` として manual review source に分離しました。

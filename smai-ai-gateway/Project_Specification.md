...
| `POST /api/v1/context-answer` | `user_question`, `context`, 任意 `constraints`, 任意 `model` | `answer`, `materials`, `cautions`, `next_checkpoints`, `referenced_sections`, `confidence`, `provider`, `model`, `elapsed_ms` | LLM がスコアや順位を変更せず、渡された context から説明補助だけを返すこと。`task_type=cockpit_interpretation` ではCockpit材料の読み解き支援に限定する。 |

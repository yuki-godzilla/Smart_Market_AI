...
| UX-014c | Research Summary / Investment hint news | 投資ヒントニュースカードを確認する | `AI調査を更新` 後に `投資ヒントとなるニュース` を確認する | URL付き一般ニュースだけがカード化され、TDnet、企業IR、EDINET、provider profile、URL不足ニュースは混ざらない。材料分類、確認観点、鮮度、要約、なぜ見るか、追加確認、ニュースURLが読め、売買推奨やランキング順位変更に見えない | ニュース・開示・provider source が混ざり、何が投資ヒントニュースか分からない | UX Readability | High | In review | UI表示限定slice実装済み。provider breadth拡張は後続 scope |

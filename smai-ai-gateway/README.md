...
Model routing lives in Gateway, not SMAI parent: clients send `task_type`, `execution_mode`, `environment_profile`, and optional `preferred_profile`; Gateway resolves `assistant_fast` / `assistant_standard` / `assistant_quality` / `report_quality` / `fallback` to provider/model/timeout/token settings and returns `profile` in responses.

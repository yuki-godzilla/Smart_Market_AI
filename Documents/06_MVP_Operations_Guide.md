...
- `上向き兆候` is an independent Ranking exploration policy. It combines chart shape 30%, forecast upside room 25%, downside safety 20%, pullback/stability 10%, company/data/dividend quality 10%, and upward material 5%, then applies hard caps for poor data quality, high downside warning, low risk confirmation, excessive drawdown, non-positive forecast, no upward models, and sharp recent decline. It never overwrites `total_score`. Ranking, Cockpit, Myウォッチリスト snapshots, Ranking History, Assistant context, and Decision Report retain backward-compatible internal `reversal_*` fields while displaying `上向き兆候`. Treat it as a further-review priority, not buy advice.
  - When selected, the Ranking condition card shows the difference from `上昇気配` and the exact 30/25/20/10/10/5 formula. `上向き兆候スコアの詳しい計算方法を見る` shows component inputs, chart-shape patterns, pullback bands, and hard-cap thresholds.

...
- `ranking_interpretation` + `response_schema=ranking_interpretation.v1`: 明示実行されたRankingの固定済み上位候補を整理する。`ranking_context_id`、候補順、candidate ID、`cited_evidence_ids`を検証し、summary / common_strengths / common_cautions / metric_notes / sector_notes / candidate_notes / next_checkpointsを返す。未知citation、候補順不一致、candidate重複はfallbackとし、順位・score・Forecastは再計算しない。
- `radar_overview_interpretation` + `response_schema=radar_overview_interpretation.v1`: 明示実行されたInvestment Radar全体を整理する。`radar_context_id`、`context_hash`、sector / theme / deep-dive candidate ID、theme-candidate関係、`cited_evidence_ids`を検証し、summary / candidate_set_movement / sector_notes / theme_notes / deep_dive_hints / unknowns / next_checkpointsを返す。stale / missing marketではmovementをnullとし、市場全体へ一般化しない。

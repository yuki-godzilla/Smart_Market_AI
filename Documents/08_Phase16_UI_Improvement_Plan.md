...
Symbol Cockpit は、Summary / KPI の直後に `02 Price & Forecast` を置き、価格・予測チャートを1銘柄深掘りの主役として扱う。ランキングで使う `上昇気配・下降警戒` をチャート直後に置き、`04 Score Breakdown` は値動きと2指標を見たあとに「なぜこの評価になっているか」を確認する位置づけにする。
...
- `03 上昇気配・下降警戒` で `上昇気配`、`下降警戒`、モデル方向一致、予測のばらつきを確認する。

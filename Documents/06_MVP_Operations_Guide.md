...
- Research Summary maturity slice として、`ResearchBrief` の前段に `ResearchFactSummary` を追加済み。運用上の表示目標は、取得状態や件数ではなく、事業概要、主要事業、確認済みのIR / 公式資料 / TDnet / ニュース、主要定量指標、直近イベント、良材料候補、注意材料候補、未確認項目を source-backed fact として提示すること。provider-only の情報は公式資料と同列に扱わず、`外部プロバイダー情報では` と明示する。

...
- Screening Score MVP: implemented with explainable score breakdown rows, beginner-friendly Japanese reason labels, Streamlit Market Data preview, JSON / CSV exports, and `POST /screening/score` / Screening Score MVP は説明可能な score breakdown 行、初心者向けの日本語理由ラベル、Streamlit Market Data preview、JSON / CSV export、`POST /screening/score` まで実装済み
- Forecast Lab Baseline: started with deterministic naive, moving-average, and momentum baseline models, walk-forward metrics, and Streamlit close-price / forecast-line preview / Forecast Lab Baseline は deterministic な naive、moving-average、momentum baseline model、walk-forward metrics、Streamlit の終値 / forecast line preview から着手済み

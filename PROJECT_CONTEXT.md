...
- Research Summary readability first slice: local rule-based `ResearchBriefBuilder` is implemented before any external LLM work. The primary UI order is AI整理メモ -> 確認ポイント -> 定量指標 -> 確認不足 -> 次に確認すべき資料, while source cards, Research Score, external source traces, and detail tables are behind expanders. AI整理メモ is a visible card with non-advice / source-confidence / missing-metric badges, quantitative metrics are small cards, and missing metrics are shown in a warning panel. The first metric extraction uses deterministic regex / keyword rules and shows missing metrics explicitly.

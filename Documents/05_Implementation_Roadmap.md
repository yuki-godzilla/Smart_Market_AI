...
| Phase N1〜N6 / Notification Platform | 🟦 **N1〜N5-C foundation実装済み / N6実データ接続待ち** | schema v5通知履歴、型付きcatalog、設定/dedupe対応Producer、専用通知センター、手動生成、opt-in scheduler/job registry/run log/runnerまで実装。実favorite/news/sector cache adapterとResearch/report event接続は次段階。 |
...
- 🟦 **N1〜N5-C foundation実装済み**: 独立gateway、親client/adapter、versioned SQLite設定/履歴、catalog、Producer、専用通知センター、manual generation、opt-in scheduler基盤を実装。N6は実favorite/news/sector cache adapterとResearch/report event接続を行う。詳細は `Documents/04_Detail_Design/04-10_Onepager_Notification_Platform.md`。
...
進捗: 20/60営業日のnetwork-free rolling-origin評価、horizon purge、model/consensus自身のfold-level誤差、最新予測、market / asset type / regime別集計、error cases、時系列holdoutで比較するweighting調整案まで実装済み。候補weightは前半originで作り、後半holdoutでRMSE改善かつ方向一致率維持の場合だけ明示適用できる。通常Rankingへは自動反映しない。calibrationと実データ評価は後続。
